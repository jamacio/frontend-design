...
A **Skill** is the atomic unit of design capability in OD. We adopt code agent's `SKILL.md` convention verbatim as the base format, then add optional fields for design-specific features (preview type, input schema, slider parameters). A skill written for plain code agent runs in OD. An OD skill that doesn't use our extensions runs in plain code agent.

> **Compatibility promise:** A skill like [`guizang-ppt-skill`] works in OD **without modification**. It just drops into `~/.AI assistant/skills/` and OD discovers it.

---

## 1. Base format (unchanged from code agent)
...
The goal: **zero-config compatibility** for existing code agent skills.
...
|---|---|---|
| `./.AI assistant/skills/` | 1 (highest) | project-private skills, not committed |
| `./skills/` | 2 | project-committed skills |
| `~/.AI assistant/skills/` | 3 | user-global skills |
...
`cc-switch` maintains a central skill dir at `~/.cc-switch/skills/` and symlinks it into each agent's expected location (`~/.AI assistant/skills/`, `~/.code agent/skills/`, etc.). OD can opt into the same model:

```
~/.frontend-design/skills/
    magazine-web-ppt/      (canonical location)
~/.AI assistant/skills/
    magazine-web-ppt → ~/.frontend-design/skills/magazine-web-ppt
~/.code agent/skills/
    magazine-web-ppt → ~/.frontend-design/skills/magazine-web-ppt
```
...
- **Typical workflow:** analyze input → draft 9 sections per awesome-AI assistant-design schema → generate sample component preview → finalize.
...
The 9-section DESIGN.md format is **not invented by OD**; it's the [awesome-AI assistant-design] convention, reproduced here for convenience:
...
```sh
od skill add 
# → clones into ~/.frontend-design/skills/magazine-web-ppt
# → symlinks into ~/.AI assistant/skills/ (and any other active agent dirs)
# → re-indexes registry
...
5. Daemon dispatches to active agent (code agent) with:
...
`od skill test <name>` runs the skill against each case using a cheap model (e.g. AI model 4.5) and asserts on the manifest + regex. Low-fidelity but catches structural regressions.
