...
- Linux release artifact bundle contains the expected AppImage, metadata, and
  uploaded `frontend-design-release-linux-e2e-report`.
...
```bash
corepack pnpm guard
corepack pnpm --filter @frontend-design/tools-pack test -- linux.test.ts
corepack pnpm --filter @frontend-design/tools-pack typecheck
corepack pnpm --filter @frontend-design/e2e test -- tests/linux-helpers.test.ts tests/packaged-smoke-workflow.test.ts
corepack pnpm --filter @frontend-design/e2e typecheck
git diff --check
```
