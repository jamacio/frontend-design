# Frontend Design Plugin & Marketplace Spec (v1)

> **In one sentence:** Frontend Design plugins turn portable `SKILL.md` capabilities into marketplace-ready, one-click design workflows while preserving compatibility with existing agent skill catalogs, headless CLI use, and self-hosted deployment.
...
A **Plugin** is the unit of distribution for Frontend Design. Where a [Skill](skills-protocol.md) describes a single capability that an agent can run, a Plugin is the shippable bundle around it: one or more skills, an optional design system reference, optional craft rules, optional AI assistant-plugin assets, a preview, a use-case query, an asset folder, and a small machine-readable sidecar that powers OD's marketplace surface. A plugin is always anchored to a portable `SKILL.md` so it is publishable to every existing skill catalog without modification.

> **Compatibility promise (extends [`skills-protocol.md`](skills-protocol.md)):** Any plugin folder that ships a `SKILL.md` works as a plain agent skill in code agent, code editor, code agent, AI assistant CLI, OpenClaw, Hermes, etc. Adding `frontend-design.json` is purely additive — it unlocks OD's marketplace card, preview, one-click "use" flow, and typed context-chip strip, but it never changes how the underlying skill runs. **One repo, two consumption modes.**
...
1. **Plugin author ships portable capability.** `SKILL.md` remains the executable agent contract; `frontend-design.json` adds OD marketplace metadata, input fields, defaults, previews, and context wiring.
...
| Question | Figma-era plugin assumption | Frontend Design v1 answer |
| --- | --- | --- |
...
sequenceDiagram
  ...
  participant P as Plugin Manifest<br/>(SKILL.md + frontend-design.json)
...
| Headless CLI / code agent | code agent, code editor, code agent, CI, or script shells out to `od` | `od run create --plugin make-a-deck --input audience=investors --input topic='AI design ops'` | Same manifest resolution, same context chips, same run events without opening desktop. |
...
**Current implementation clarification:** `discovery -> plan -> generate -> critique` is a reference pipeline shape, not a fixed hard-coded wizard. A plugin snapshot can carry `od.pipeline.stages[].atoms[]`; the daemon resolves that snapshot, injects the active plugin block plus active stage atom blocks into the system prompt, emits stage events, and lets the agent work through the pipeline. When the user has not explicitly selected a plugin, OD still does **not** launch a generic naked agent: the base Frontend Design designer prompt and discovery rules are always present. Product entry points bind sensible defaults on top of that base: Home free-form input routes through the bundled, hidden `od-default` scenario, while typed New Project flows choose the default bundled scenario for the project kind. `od-default` is a router and task-shaper; it should guide the run into the normal design pipeline, not be treated as a standalone "make it beautiful" aesthetic engine.
...
5. [`frontend-design.json` schema](#5-frontend-designjson--schema-v1)
6. [`frontend-design-marketplace.json` schema](#6-frontend-design-marketplacejson--federated-catalog)
...
10. [First-party atoms](#10-first-party-atoms--frontend-designs-atomic-capabilities)
...
13. [Public web surface](#13-public-web-surface-frontend-designaimarketplace)
...
19. [Why this is a meaningful step for Frontend Design](#19-why-this-is-a-meaningful-step-for-frontend-design)
...
- Defaults locked from the planning round (overridable in review):
  - **Compatibility = wrap-then-extend.** Existing `SKILL.md` and `.AI assistant-plugin/plugin.json` repos run as-is; `frontend-design.json` is an additive sidecar.
...
1. Every runnable, distributable OD plugin is a valid agent skill (`SKILL.md`- or `.AI assistant-plugin/plugin.json`-anchored). No fork of the skill spec.
2. A vanilla skill or AI assistant-plugin repo becomes an OD plugin by adding an optional `frontend-design.json` sidecar — no rename, no body changes.
3. Three install sources: local folder, GitHub repo (with optional ref/subpath), arbitrary HTTPS archive, plus federated `frontend-design-marketplace.json` indexes.
4. One-click "use" auto-fills the brief input and a strip of `ContextItem` chips above it (skills, design-system, craft, assets, MCP, AI assistant-plugin, atom).
5. Tiered trust by default; capability scoping is declarative and optional.
6. The OD core engine, atomic capabilities, and plugin runtime are all reachable from CLI so any code agent can drive Frontend Design headlessly.
...
- Replacing SKILL.md / AI assistant-plugin spec — OD never forks.
...
| File present                                                       | OD installs    | code agent / code editor / code agent / AI assistant CLI | OpenClaw / Hermes | awesome-agent-skills | clawhub | # |
| ------------------------------------------------------------------ | -------------- | ----------------------------------------- | ----------------- | -------------------- | ------- | --------- |
| `SKILL.md` only                                                    | yes            | yes                                       | yes               | yes                  | yes     | yes       |
| `.AI assistant-plugin/plugin.json` only                                  | yes            | yes (AI assistant)                              | partial           | listable             | listable| listable  |
| `frontend-design.json` only                                            | metadata-only  | no                                        | no                | no                   | no      | no        |
| `SKILL.md` + `frontend-design.json`                                    | enriched       | yes                                       | yes               | yes                  | yes     | yes       |
| `.AI assistant-plugin/...` + `frontend-design.json`                          | enriched       | yes (AI assistant)                              | partial           | listable             | listable| listable  |
| `SKILL.md` + `.AI assistant-plugin/...` + `frontend-design.json`             | fully enriched | yes                                       | yes               | yes                  | yes     | yes       |

The takeaway: **`SKILL.md` is the lowest common denominator**. Every plugin recommended for distribution should ship a `SKILL.md` so it lands cleanly in every major catalog, then add `frontend-design.json` to gain OD's product surface.

A folder that contains only `frontend-design.json` is not a runnable plugin in v1; it is a **metadata-only preset**. OD may read it to show a marketplace card, aggregate remote references, or act as a future install stub, but it cannot trigger an agent run and cannot be listed in cross-agent catalogs. `od plugin doctor` must mark this shape as `metadata-only` and prompt the author to add `SKILL.md` or `.AI assistant-plugin/plugin.json` before publishing it as a runnable plugin.
...
├── .AI assistant-plugin/                   # optional: AI assistant-plugin compat (commands/agents/hooks/.mcp.json)
│   └── plugin.json
├── frontend-design.json                  # optional sidecar — unlocks OD product surface
...
└── plugins/                          # nested AI assistant-plugins (bundle plugins)
```

Rules of authorship:

- `SKILL.md` body never carries OD-specific metadata; it stays clean and portable.
- `frontend-design.json` only ever **points** at SKILL.md / DESIGN.md / craft files; it never duplicates their bodies.
- Existing OD-specific frontmatter on SKILL.md (the `od:` namespace already documented in [`skills-protocol.md`](skills-protocol.md) and used in [`skills/blog-post/SKILL.md`](../skills/blog-post/SKILL.md)) is honored as a fallback for plugins without `frontend-design.json`. We do not deprecate it; we layer over it.
- A runnable v1 plugin must contain at least one of `SKILL.md` or `.AI assistant-plugin/plugin.json`. `frontend-design.json` does not define agent behavior by itself; it only tells OD how to display, resolve, and apply that behavior.

## 5. `frontend-design.json` — schema v1
...
```json
{
  ...
  "author":   { "name": "Frontend Design", "url": "#" },
  "license":  "MIT",
  "homepage": "
  "icon":     "./icon.svg",
  ...
  "compat": {
    ...
    "AI assistantPlugins": [{ "path": "./.AI assistant-plugin/plugin.json" }]
  },
  ...
  "od": {
    ...
    "context": {
      ...
      "AI assistantPlugins":  [{ "ref": "./plugins/code-review" }],
      "mcp": [
...
- `specVersion` — the Frontend Design plugin spec version used to interpret the manifest. This is distinct from plugin `version` and is frozen into apply snapshots for replay.
...
export type ContextItem =
  ...
  | { kind: 'AI assistant-plugin'; id: string; label: string }
...
| `mcp`                  | Daemon writes `.mcp.json` during run start so tool servers from the plugin start |
| `subprocess` / `bash`  | AI assistant-plugin hooks execute, agent-callable bash tools enabled                |
...
- Declaring `.AI assistant-plugin` hooks implies `subprocess`; if a hook reads bundled assets, it also requires `fs:read`.
...
- Provider credentials are not a v1 capability. Plugins cannot directly declare access to `AI_API_KEY`, media provider keys, or connector secrets. Capabilities that need credentials must go through OD-owned first-party atoms / tools.

### 5.4 `SKILL.md` frontmatter to `PluginManifest` mapping

When a plugin has no `frontend-design.json`, but its `SKILL.md` already contains the `od:` frontmatter defined in [`skills-protocol.md`](skills-protocol.md), `adapters/agent-skill.ts` synthesizes a minimal `PluginManifest`. The mapping must be stable so the legacy skill protocol and the new plugin schema do not drift into two semantics:
...
If `frontend-design.json` and `SKILL.md` frontmatter both exist, `frontend-design.json` wins, but the loader must preserve adapter warnings. Authors can migrate incrementally: first keep the old skill runnable as-is, then add OD marketplace metadata.

## 6. `frontend-design-marketplace.json` — federated catalog

Mirrors [`AIs/skills/.AI assistant-plugin/marketplace.json`](# assistant-plugin/marketplace.json) so existing community catalogs need only a rename to be reusable.
...
```json
{
  ...
  "version": "1.0.0",
  "owner":    { "name": "Frontend Design", "url": "#" },
  "metadata": { "description": "First-party plugins", "version": "1.0.0" },
  "plugins": [
    { "name": "make-a-deck", "version": "1.0.0", "source": "github:frontend-design/plugins/make-a-deck", "tags": ["deck"] },
    { "name": "tweet-card",  "version": "1.0.0", "source": "https://files.../tweet-card-1.0.0.tgz",  "tags": ["marketing"] }
  ]
}
```

The marketplace top-level `version` is the catalog snapshot version; every `plugins[]` entry also declares the listed plugin version. Installers still verify the target folder's own `frontend-design.json` after fetching, but registry search, audit logs, and marketplace refresh events can now reason about catalog and plugin versions before install.
...
| -------- | ------------------------------------------------ | ------------------ | ---------------------------------------------------------------------- |
| 1        | `<projectCwd>/.frontend-design/plugins/<id>/`        | plugin bundle      | New; explicitly installed into the project and committed with user code |
| 2        | `<projectCwd>/.AI assistant/skills/<id>/`              | legacy `SKILL.md`  | Keeps the project-private skill path from [`skills-protocol.md`](skills-protocol.md) compatible |
| 3        | `<daemonDataDir>/plugins/<id>/`                  | plugin bundle      | New; written by `od plugin install` under the daemon data root          |
| 4        | `~/.frontend-design/skills/<id>/`                    | legacy `SKILL.md`  | OD canonical skill install path; may symlink into other agents          |
| 5        | `~/.AI assistant/skills/<id>/`                         | legacy `SKILL.md`  | Compatibility scan for external code agent / skills tooling            |
| 6        | repo root `skills/`, `design-systems/`, `craft/` | bundled resources  | Existing first-party resources, unchanged                              |

Conflict resolution uses normalized `name` / plugin id; lower numeric priority wins. Legacy `SKILL.md` locations are synthesized into plugin records by the adapter, but are not copied into `<daemonDataDir>/plugins/` unless the user explicitly runs `od plugin install`. This keeps existing AI assistant skills zero-config while giving plugin bundles a clear install root.
...
od marketplace add https://.../frontend-design-marketplace.json
```

GitHub install path uses `codeload install path`, no git binary required, with path-traversal guards and a configurable size cap.
...
export interface AppliedPluginSnapshot {
  ...
  mcpServers: McpServerSpec[];                  // tool server set that was active at apply time; same payload as ApplyResult.mcpServers but frozen
}
...
flowchart LR
  ...
  C -->|"+ subprocess / bash"| P4[register AI assistant plugin commands/hooks]
...
export interface GenUISurfaceSpec {
  ...
  oauth?: {
    ...
    mcpServerId?: string;                      // required when route='mcp'; references a name from the plugin's tool server set
  };
}
```
...
| `mcp` | Reuses `POST /api/mcp/oauth/start`; the token lands in `<dataDir>/mcp-tokens.json` | Reuses the Settings → tool servers OAuth visuals | `genui_surfaces.value_json = { mcpServerId }`; the token never enters SQLite |
| `plugin` (Phase 4) | Plugin supplies arbitrary third-party OAuth metadata; daemon goes through a generic PKCE adapter | TBD | TBD |

`od plugin doctor` enforces at install / apply time that: (1) when `oauth.route === 'connector'`, `oauth.connectorId` is present in the plugin's own `od.connectors.required[]` or `od.connectors.optional[]`; (2) when `oauth.route === 'mcp'`, `oauth.mcpServerId` matches a name in the plugin's tool server set.
...
| Frontend SDK compatibility | OD desktop / web with built-in renderer | `@frontend-design/agui-adapter` lets AI assistantKit / other AG-UI clients consume an OD run unchanged |

The adapter is an interoperability surface, not the internal UI source of truth. OD should not add AI assistantKit as a required product dependency unless a separate external embed/demo/client explicitly needs it. v1 plugins need no change to be consumable inside the AG-UI ecosystem because the adapter is a projection of OD's own events.
...
- `parsers/manifest.ts` — read `frontend-design.json` → `PluginManifest` (Zod-validated).
...
- `adapters/AI assistant-plugin.ts` — read `.AI assistant-plugin/plugin.json` → synthesize a `PluginManifest`.
- `merge.ts` — merge sidecar + adapters with `frontend-design.json` winning; foreign content lands in `compat.*`.
...
CREATE TABLE installed_plugins (
  ...
  manifest_json        TEXT NOT NULL,    -- cached frontend-design.json (or synthesized)
...
CREATE TABLE applied_plugin_snapshots (
  ...
  mcp_servers_json        TEXT NOT NULL DEFAULT '[]',  -- tool server set active at apply time, frozen as McpServerSpec[]
...
> **Transport equivalence (rule of thumb).** Every endpoint above is also exposed as a CLI subcommand and, where it fits MCP semantics, as an tool integration. Code agents should use the CLI; only the desktop web app and `od …` itself use HTTP directly. See §12 for the full CLI surface.
...
OD runs in three operating modes that share **one** daemon, **one** CLI, and **one** plugin runtime. The differences are purely presentational. This is the same shape code editor uses: `code editor-agent` (CLI) is sufficient on its own; the IDE is sugar.

| Mode                | What runs                                              | When to use                                          | Entry                                            |
| ------------------- | ------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------ |
| **Headless**        | Daemon process only — no web bundle, no electron       | CI, servers, containers, AI assistant-Code-driven flows    | `od daemon start --headless` (new flag, Phase 2) |
...
- A user with **only code agent** (or any code agent) plus `npm i -g @frontend-design/cli` plus a running headless daemon can do the entire user journey: install plugin → create project → run → consume artifacts. No OD desktop required.
...
- CI is a first-class citizen: a GitHub Action can `npm i -g @frontend-design/cli && od daemon start --headless && od plugin install … && od run start --project … --follow`. No display, no electron, no per-step UI scripting.
...
od run start --project <projectId> [--conversation <conversationId>]
             ...
             [--agent AI assistant|code agent|AI assistant] [--model <id>] [--reasoning <level>]
...
od plugin export    <projectId> --as od|AI assistant-plugin|agent-skill --out <dir>
od plugin doctor    <id>
od plugin scaffold
```
...
od tools connectors  ...
od mcp                       # stdio tool server
od mcp live-artifacts        # specialized tool server
```
...
A code agent driving Frontend Design through the CLI typically does:
...
## 13. Public web surface (#/marketplace)

The product site already lives at [#](#). The public marketplace ships as a path on that same site — `#/marketplace` (canonical) with `#/plugins` as an alias — not as a separate domain. It is a static-rendered catalog rendered from the official `frontend-design-marketplace.json` index, with plugin detail pages backed by the same `frontend-design.json` files inside each listed repo. Visually it mirrors what [`#`](#) does for skills, but its detail pages render OD-specific previews (the `od.preview.entry` HTML, sample outputs, the use-case query, the chip preview).

The site shares one source of truth with the in-app marketplace:

- Same JSON Schemas (`#`, `#`).
- Same federated listing format (`frontend-design-marketplace.json`).
- Same plugin manifests (`frontend-design.json` inside each repo).
...
| Public marketplace (`#/marketplace`)      | Anonymous visitors, SEO, share | Deep-link `od://plugins/<id>?apply=1` (auto-installs and applies in the desktop app), plus "Copy install command" |
...
The desktop app registers the `od://` URL scheme; clicking a button on `#/marketplace` either launches the desktop or, if it is not installed, falls back to a "How to install Frontend Design" flow.
...
| ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- | ----------------------------------------------------- |
| [`AIs/skills`]                                      | `SKILL.md` with `name`/`description` frontmatter                          | `SKILL.md` (canonical anchor)                          |
| [`AIs/AI assistant-code/plugins`]  | `.AI assistant-plugin/plugin.json` + standard structure                          | `.AI assistant-plugin/plugin.json` (optional)                |
| [`VoltAgent/awesome-agent-skills`]            | PR adds a row pointing at the repo URL                                     | repo URL — automation in §14.1                         |
| [`openclaw/clawhub`]                                        | submission via clawhub web app or PR                                       | repo URL                                                |
| [`#`](#)                                                                | indexed automatically once `npx skills add owner/repo` is observed         | repo URL                                                |
| `frontend-design-marketplace.json`                                                                   | entry referencing `github:owner/repo`                                      | `frontend-design.json` enriches the listing                |

### 14.1 Author tooling

- `od plugin scaffold` — writes a starter folder containing both `SKILL.md` (industry-standard, with `od:` frontmatter for backward compat) and `frontend-design.json` (OD enrichment with `compat.agentSkills` pointing at the SKILL.md).
- `od plugin doctor` — runs the JSON Schema, the SKILL.md frontmatter parser, and a "does this look listable on awesome-agent-skills / clawhub / #?" lint that checks for README presence, license file, and frontmatter completeness.
...
1. **Skill-only consumption (no OD).** A code editor user runs `npx skills add frontend-design/make-a-deck`. code editor reads `SKILL.md` and runs the workflow. No OD CLI, no OD daemon. The plugin's marketplace polish (`frontend-design.json`) is ignored — code editor sees a vanilla skill.
2. **Headless OD (CLI + code agent, no OD UI).** A power user keeps using their preferred code agent — code agent, code editor, code agent, etc. — but adds OD as a side service to gain plugin context resolution, project bookkeeping, design library injection, and artifact tracking. No browser, no electron. See §14.3 below for the concrete pipeline.
...
### 14.3 Concrete headless pipeline (code agent + `od` CLI, no OD UI)

This mirrors what code editor-agent + scripts can do for code editor — code agent does the thinking, OD CLI provides the project / plugin / artifact substrate.

```bash
# One-time setup: install the OD CLI as an npm global (publishable as @frontend-design/cli).
...
od plugin install github:frontend-design/plugins/make-a-deck
...
# Drive the run with code agent (or any code agent). Two equivalent paths:

# Path A — let `od` orchestrate code agent as the run's agent:
od run start --project "$PID" --plugin make-a-deck \
             --agent AI assistant --follow

# Path B — drive code agent directly inside the project cwd; OD only provides
...
code agent "Read .od-skills/ and produce the deliverables the active plugin describes."
...
### 14.4 Analogy: code editor vs `code editor-agent`, OD desktop vs `od` CLI

The mental model:

| Layer                 | code editor                                       | Frontend Design                                          |
| --------------------- | -------------------------------------------- | ---------------------------------------------------- |
| Headless agent CLI    | `code editor-agent` (drives the agent loop)       | `od run start --agent AI assistant --follow` + `od plugin run` |
| Local services / db   | code editor's background indexing / state         | OD daemon, SQLite, `.od/projects/<id>/`              |
| GUI productivity layer| code editor IDE                                   | OD desktop / web UI (`apps/web` + `apps/desktop`)    |
| Plugin / skill format | `.code editor/rules/`, tool servers                | `SKILL.md` + `frontend-design.json` + atoms              |
...
- **Tag**: `ghcr.io/frontend-design/od:<version>` plus moving `:latest` and `:edge`.
...
- **Contents**:
  ...
  - Bundled code-agent CLIs that OD supports as agent backends: code agent, code agent CLI, AI assistant CLI. Selectable per run; default is `OD_AGENT_BACKEND`.
...
OD_AGENT_BACKEND=AI assistant              # default code agent backend
...
AI_API_KEY=...                # provider keys; also storable via `od config set`
TAVILY_API_KEY=...
```
...
```bash
docker run --rm -p 17456:17456 ghcr.io/frontend-design/od:latest
...
docker run -d --name od \
  ...
  ghcr.io/frontend-design/od:latest
```

Reach the same surfaces inside the container:

```bash
docker exec od od plugin install github:frontend-design/plugins/make-a-deck
...
1. **Self-hosted enterprise.** A company hosts a private OD instance, registers an internal `frontend-design-marketplace.json` (`od marketplace add https://internal/...`), restricting plugins to internally vetted ones. Their designers and PMs use the desktop client locally; their CI uses `docker exec od od …`.
...
- JSON Schemas at `docs/schemas/frontend-design.plugin.v1.json` and `frontend-design.marketplace.v1.json`.
...
- `pnpm --filter @frontend-design/plugin-runtime test` (parser fixtures: pure SKILL.md, pure AI assistant plugin, metadata-only frontend-design.json, all three combined, SKILL frontmatter mapping).
- `pnpm --filter @frontend-design/daemon test`. `pnpm guard`, `pnpm typecheck`.
...
Validation: install plugin from a local mock marketplace.json, rotate ref, uninstall. Restricted plugin cannot start tool server until "Grant" is clicked.

### Phase 4 — Atoms, publish-back, full CLI parity (1–2 weeks, splittable)

- Document atoms in `docs/atoms.md`; expose via `GET /api/atoms`.
- `od plugin export <projectId> --as od|AI assistant-plugin|agent-skill` — generates a publish-ready folder from an existing project.
- `od plugin run <id> --input k=v --follow` — shorthand wrapper for apply + run start + watch.
- `od plugin scaffold` interactive starter.
- `od plugin publish --to AIs-skills|awesome-agent-skills|clawhub` opens a PR template.
...
- **§10.3.5 full AG-UI alignment:**
  - New package `@frontend-design/agui-adapter` — bidirectionally maps OD's `PersistedAgentEvent` + `GenUIEvent` onto AG-UI canonical events (`agent.message`, `tool_call`, `state_update`, `ui.surface_requested`, `ui.surface_responded`).
  - Daemon adds an optional `/api/runs/:runId/agui` SSE endpoint that emits AG-UI canonical events so AI assistantKit / other AG-UI clients can consume an OD run unchanged.
  ...
  - Open-Ended (MCP-Apps / Open-JSON) mode: plugins push arbitrary JSON UI trees through an tool server, rendered by desktop / web under a constrained schema.
...
- **Per-cloud one-click templates** in a separate `frontend-design/deploy` repo (CloudFormation, Deployment Manager, ARM, Aliyun ROS, Tencent TIC, Huawei RFS) — non-blocking; track separately.
...
OD reads it as a plugin via the existing `od:` frontmatter loader documented in [`skills-protocol.md`](skills-protocol.md). No `frontend-design.json` needed — the plugin lacks marketplace polish but is fully runnable.
...
`SKILL.md` stays portable — code editor / code agent / OpenClaw read it directly. `frontend-design.json` adds preview, query, chip strip, capabilities. The repo lists cleanly on every catalog in §14 without modification.
...
├── frontend-design.json                  # kind: 'bundle'; lists nested skills
├── skills/
...
| Risk                                                        | Mitigation                                                                                          |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Schema drift between OD plugin and the broader skill spec   | `frontend-design.json` is sidecar-only; it never modifies SKILL.md. CI tests run against the public AIs/skills repo. |
...
| Project-local plugins committed to user repos | Discovery only at `<projectCwd>/.frontend-design/plugins/`; opt-in via `od plugin install --project`. |
...
| Plugins shipping their own tool servers may fail to start | `od plugin doctor` runs a dry-launch of declared MCP commands; failures surfaced before "Use". |
...
| Multi-cloud testing matrix is large                         | Phase 5 ships a single canonical compose smoke (one cloud), then adds clouds incrementally; per-cloud one-click templates live in `frontend-design/deploy` and can move at their own cadence (§15.5). |
...
| AG-UI ecosystem may evolve, drifting OD's wire format from canonical AG-UI | OD-native `GenUIEvent` remains the internal source of truth. `@frontend-design/agui-adapter` is an external projection layer, so upstream protocol revs do not couple to the daemon or web renderer release cadence. |
...
- **Marketplace JSON shape** — diverge from AI's `marketplace.json` shape, or stay byte-compatible so existing AI assistant-plugin marketplaces are reusable as-is? (Default: stay byte-compatible.)
...
- ~~**Timing of AG-UI protocol adoption**~~ — **resolved.** `@frontend-design/agui-adapter` and `GET /api/runs/:runId/agui` have shipped as optional interoperability. OD-native GenUI remains the internal renderer and AI assistantKit is not a required product dependency.

## 19. Why this is a meaningful step for Frontend Design

- **Inherited supply.** Every public agent skill on `AIs/skills`, `awesome-agent-skills`, `clawhub`, and `#` is one optional `frontend-design.json` away from being an OD plugin — and reciprocally, every OD plugin is publishable to all four catalogs without modification.
...
- "Design → production code" in v1 is a **two-product handoff**: OD owns the design substrate (SKILL.md / DESIGN.md / craft / generated artifacts staged into project cwd, plus `od files`-managed artifact bookkeeping); the user's existing code agent (code editor / code agent / code agent / AI assistant CLI) owns the actual repo patch in the user's repo cwd.
...
2. **The user's code agent operates in that cwd or in their own repo cwd.** OD does not run inside the IDE; it runs as a daemon next to the IDE. code editor / code agent / code agent / AI assistant CLI are the patch-applying surface.
...
| --- | --- | --- |
| Call a tool OD does not provide (Figma REST, AST parsing, SVG conversion, etc.) | Bundle an tool server in `od.context.mcp[]` | §5 / §5.3 (`mcp` + `subprocess` + `network`) |
...
Phrased as a rule: **OD-native atom missing → plugin authors compose `tool server + bash + SKILL.md` to substitute**. The cost is ergonomics (each plugin re-invents its own naming and prompt fragments), not capability.
...
- Last stage emits a `confirmation` GenUI surface: "ready to apply? Open the project cwd in code editor / code agent / code agent and run the included instructions."
...
2. **Stabilize the contract.** Once the SKILL.md instructions, tool integration surface, and pipeline shape stabilize, distill them into a candidate first-party atom contract: atom id, prompt fragment, tool gating, optional `until` signals, optional GenUI surface defaults.
...
| --- | --- | --- |
| **A. Userspace (movable to plugin)** | A first-party plugin's manifest + SKILL.md | atom prompt fragments, default reference pipelines, atom-bundled tool servers, critique scoring axes, discovery question wording |
...
| **C. Already plugin-driven in v1** | Already in `<daemonDataDir>/plugins/...` or per-project plugin folders | Active SKILL.md, DESIGN.md, craft .md, plugin-declared tool servers, plugin-declared GenUI surfaces, plugin-declared pipelines |
...
- Plugin-declared tool servers, connector requirements, GenUI surfaces, and `od.pipeline.stages[]` are all consumed via the same registry and resolver paths that a third-party plugin uses.
...
- `@frontend-design/agui-adapter` and `/api/runs/:runId/agui` provide external AG-UI event projection without changing OD's internal GenUI renderer.
...
- An atom plugin is a folder with `frontend-design.json` (`od.kind: 'atom'`) plus `SKILL.md`.
- The `SKILL.md` body is the atom's prompt fragment, injected by the daemon assembler when a stage references the atom by id.
- Optional `od.context.mcp[]` declares tool integrations the atom uses (e.g. `live-artifact`, `connector`).
...
- `plugins/_official/scenarios/od-new-generation/frontend-design.json`
- `plugins/_official/scenarios/od-figma-migration/frontend-design.json` (after Phase 6)
- `plugins/_official/scenarios/od-code-migration/frontend-design.json` (after Phase 7)
- `plugins/_official/scenarios/od-tune-collab/frontend-design.json`
