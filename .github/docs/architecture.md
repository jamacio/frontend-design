...
│                       │                            │
│                       ▼                            │
│            spawns: AI assistant / code agent / code editor / …     │
└────────────────────────────────────────────────────┘
```
...
browser ──► od.yourdomain.com (Vercel)
        ...
        od daemon on user's laptop
              │
              ▼
        spawns: AI assistant / code agent / …
```
...
browser ──► od.yourdomain.com (Vercel serverless)
                       │
                       ▼
              AI Messages API (BYOK stored in browser)
```

No local CLI, no daemon. Degraded experience — no code agent skills, no filesystem artifacts (stored in IndexedDB), no PPTX export. But it's the "just try it" path. Keys stored `localStorage` with explicit warning.
...
- Scan and index **skills** from `~/.AI assistant/skills/`, `./skills/`, `./.AI assistant/skills/` on startup and on FS-watch events.
...
|---|---|---|
| `./.AI assistant/skills/` | highest | project-private skills |
| `./skills/` | medium | project-declared skills |
| `~/.AI assistant/skills/` | lowest | user-global skills |
...
- Parses the 9-section format (see [awesome-AI assistant-design][acd] schema).
...
7. Daemon re-invokes agent with surgical-edit instruction + the note.
   Adapter translates based on capabilities:
     - code agent → native tool loop, edits that region only
     - code agent → regenerates the file with "only change element X" constraint
     - API fallback → same as code agent path
```
...
| File | Purpose |
|---|---|
| `~/.frontend-design/config.toml` | daemon-global: default agent preference, keys (optional, BYOK), telemetry opt-in (default off) |
| `~/.frontend-design/agents.json` | cached agent detection results |
| `./.od/config.json` | project-local: active design system, preferred skills, preferred mode |
| `./skills/<skill>/SKILL.md` | skill manifest (standard code agent format) |
| `./DESIGN.md` | active design system ([awesome-AI assistant-design][acd] format) |
...
etc.), mirroring how code editor / code agent / code agent behave.
...
services:
  daemon:
    image: AI provider assistantdesign/daemon
    volumes: [ "~/.frontend-design:/root/.frontend-design", "./:/workspace" ]
    ports: ["7456:7456"]
  web:
    image: AI provider assistantdesign/web
    ports: ["3000:3000"]
...
| Agent running on user's machine | Agent reads/writes outside project | Adapter sets `cwd` to artifact dir; relies on agent's own permission system (code agent's `--allowed-tools` etc.) |
...
| Skill from untrusted source | Malicious skill in `~/.AI assistant/skills/` | Install-time warning; skills run under the agent's permission model, not ours |
| Vercel web bundle | Compromised build | Standard Vercel integrity; bundle has zero secrets |

We inherit the agent's permission model on purpose — we don't invent our own sandbox, because code agent's `--permission-mode` / code agent's sandboxing / code editor's containment already exist and are maintained.
