...
Frontend Design runs on Windows natively, but the path is less travelled than macOS, Linux, or WSL2. This guide covers the most common errors you will hit on a fresh Windows machine and the exact fix for each.
...
```text
Frontend Design dev server ready
...
OpenCode is one of the local agent CLIs Frontend Design can drive. If you want to use it:
...
If Frontend Design still shows OpenCode as *not installed* in **Settings → Execution mode**, click **Rescan** after confirming the `opencode.cmd` directory is on your user `PATH`.
