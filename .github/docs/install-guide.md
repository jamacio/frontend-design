...
Deploy Frontend Design on Linux or macOS with a single command. The installer wraps the existing Docker Compose stack — no build step required.
...
|----------|----------------|---------|
| Docker Engine | 24.0 | [docs.docker.com/engine/install](#) |
| Docker Compose plugin | 2.20 | Bundled with Docker Desktop; `apt install docker-compose-plugin` on Linux |
| Docker Desktop (macOS/Windows) | 4.25 | [docker.com/products/docker-desktop](#) |
...
[frontend-design] OS: Linux ubuntu 24.04 (x86_64)
[frontend-design] Docker: Docker version 26.1.3, build b72abbb
[frontend-design] Compose: Docker Compose version v2.27.1

Docker image [docker.io/vanjayak/frontend-design:latest]:
Port [7456]:
Allowed origins (CORS, comma-separated, or empty) []:
Memory limit [384m]:

[frontend-design] Pulling image: docker.io/vanjayak/frontend-design:latest
[frontend-design] Starting Frontend Design...
[frontend-design] Waiting for health check (up to 60s)...
[frontend-design] Daemon is healthy (200 OK)
```
...
|--------|---------|-------|
| **Docker image** | `docker.io/vanjayak/frontend-design:latest` | Pin a digest for reproducibility: `docker.io/vanjayak/frontend-design@sha256:<digest>` |
...
```bash
# Check status
systemctl --user status frontend-design

# Start / stop / restart
systemctl --user start frontend-design
systemctl --user stop frontend-design
systemctl --user restart frontend-design

# View logs
journalctl --user -u frontend-design -f

# Disable auto-start
systemctl --user disable frontend-design

# Re-enable auto-start
systemctl --user enable frontend-design
```
...
Docker Desktop manages the container lifecycle. Use Docker Desktop's dashboard to start, stop, or restart the `frontend-design` container, or use the CLI:
...
```bash
bash deploy/scripts/update.sh --image=docker.io/vanjayak/frontend-design@sha256:<digest>
```
...
> **Data:** By default, the `frontend_design_data` volume (projects, artifacts, config) is also deleted. Pass `--keep-data` to preserve it. Remove the volume manually later: `docker volume rm frontend_design_data`.
...
|----------|---------|-------------|
| `FRONTEND_DESIGN_IMAGE` | `docker.io/vanjayak/frontend-design:latest` | Full image reference |
| `FRONTEND_DESIGN_PORT` | `7456` | Host-side port (bound to `127.0.0.1`) |
| `FRONTEND_DESIGN_ALLOWED_ORIGINS` | _(empty)_ | CORS origins for reverse-proxy setups |
| `FRONTEND_DESIGN_MEM_LIMIT` | `384m` | Container memory cap |
