...
This document records how to read the new Frontend Design Langfuse trace shape for
...
- PR: 
- Issue: 
- Full UI validation, alignment turn: #
- Full UI validation, artifact turn: #
...
content should remain in Frontend Design object storage and be referenced by
...
Raw sensitive content belongs in Frontend Design object storage with access control,
...
Issue #3733 targets the first durable bridge between Frontend Design-owned object
...
Frontend Design-owned project storage today and to a ProjectStorage or
...
| Storage reference | Emits stable `od://objects/...` references. | Treats `storage_ref` as an Frontend Design registry reference, future-compatible with ProjectStorage/S3 backends. | No conflict; current refs should remain non-dereferenceable from Langfuse. |
...
1. Make the manifest builder resolve through a concrete Frontend Design object
...
4. Preserve `storage_ref` as a stable, non-secret reference and require any
   future download or preview access to go through Frontend Design-controlled
   authorization.
...
- current branch: `code agent/ai-native-observability-loop-spec`
...
- `pnpm install`
- `pnpm --filter @frontend-design/daemon exec vitest run -c vitest.config.ts tests/langfuse-trace.test.ts tests/langfuse-bridge.test.ts tests/AI assistant-stream-thinking.test.ts`
- `pnpm --filter @frontend-design/daemon typecheck`
- `pnpm guard`

End-to-end validation:

- UI onboarding completed on an isolated data directory.
- Real code agent runtime selected.
...
1. Store complete raw/redacted prompt payloads in Frontend Design object storage and
