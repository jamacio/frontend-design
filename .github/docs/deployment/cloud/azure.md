...
This guide deploys the Docker image to Azure Container Instances (ACI) with an Azure Files share mounted at `/app/.od` for persistent Frontend Design data.
...
```bash
export RESOURCE_GROUP=frontend-design-aci
export LOCATION=eastus
export DEPLOYMENT_NAME=frontend-design-aci
export DNS_LABEL=frontend-design-$RANDOM
...
server {
  ...
  location /api/ {
    ...
    gzip off;
    proxy_pass http://frontend_design_aci;
  }
...
az deployment group create \
  ...
  --parameters \
    ...
    dnsNameLabel="$DNS_LABEL" \
    allowedOrigins="$BROWSER_ORIGIN" \
    image="docker.io/vanjayak/frontend-design:latest" \
    cpuCores=1 \
    memoryInGB=1 \
    fileShareQuotaGB=10
```
...
- Update `resourceGroupName`, `location`, `frontendDesignImage`, and `browserOrigin`.
