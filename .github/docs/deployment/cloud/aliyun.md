...
This guide covers self-hosting Frontend Design on Alibaba Cloud for users in mainland China and the broader Asia-Pacific region. It documents the supported deployment paths, image-pull optimisations, and ICP filing considerations for public-facing instances.
...
For users in mainland China, deploying Frontend Design to Alibaba Cloud (阿里云) instead of an overseas provider gives you:
...
- Local install of the [Alibaba Cloud CLI (`aliyun`)](#) if you plan to script provisioning.

## Path A — Deploy to ECS

This path puts Frontend Design on a single ECS instance using the same Docker Compose stack documented in [`docs/deployment/docker.md`](../docker.md).
...
| Image | Ubuntu 24.04 LTS 64-bit | Frontend Design's Docker image is `linux/amd64` and `linux/arm64` |
...
echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] # $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list
sudo apt-get update
...
>
> Also note that the daemon reads `OD_ALLOWED_ORIGINS` directly. The `FRONTEND_DESIGN_ALLOWED_ORIGINS` name documented in `deploy/.env.example` is a Compose-only alias mapped in `deploy/docker-compose.yml`; for the direct-container ACK path you must set `OD_ALLOWED_ORIGINS` instead.

Create the API-token secret first:

```bash
# Generate a random token and store it in the cluster
kubectl create secret generic frontend-design-secrets \
...
spec:
  replicas: 1  # Frontend Design uses local SQLite; multi-replica needs shared storage
  selector:
    matchLabels: { app: frontend-design }
  template:
    metadata:
      labels: { app: frontend-design }
    spec:
      containers:
        - name: frontend-design
          image: #/<your-namespace>/frontend-design:latest
          ports:
            - containerPort: 7456
          env:
            - name: OD_DATA_DIR
            ...
            - name: OD_API_TOKEN
              valueFrom:
                secretKeyRef:
                  name: frontend-design-secrets
                  key: api-token            # required whenever OD_BIND_HOST is non-loopback
            - name: OD_ALLOWED_ORIGINS
              value: "https://design.example.cn"  # set when fronting with an Ingress; daemon reads OD_*, not FRONTEND_DESIGN_*
          volumeMounts:
            - name: data
              mountPath: /data
          readinessProbe:
      ...
      volumes:
        - name: data
          persistentVolumeClaim:
            claimName: frontend-design-data
---
apiVersion: v1
kind: Service
metadata:
...
Apply with `kubectl apply -f frontend-design.yaml`. Front the Service with an Ingress (NGINX Ingress Controller is preinstalled on ACK Pro) and an ACM-issued certificate. With `OD_API_TOKEN` set, every `/api/*` request from non-loopback origins must carry an `Authorization: Bearer <token>` header — wire that into your Ingress / proxy auth layer.
...
Resource Orchestration Service (资源编排 ROS) provisions the same resources declaratively. There is no first-party ROS template for Frontend Design yet. Operators with Alibaba Cloud access are welcome to contribute one as a follow-up to this guide; the natural location is `deploy/aliyun/ros/`.
...
3. Copy your personal accelerator URL (looks like `#).
...
Frontend Design's daemon binds to `127.0.0.1:7456` and is never exposed directly. Public access must go through:

1. A reverse proxy that terminates TLS (Nginx, Caddy, Alibaba Cloud SLB/ALB).
2. An ICP-filed domain (for mainland regions).
3. CORS allowlist set via `FRONTEND_DESIGN_ALLOWED_ORIGINS` (Compose / `deploy/.env` path) or `OD_ALLOWED_ORIGINS` (direct-container / Kubernetes path).
...
- Frontend Design Docker deployment: [`docs/deployment/docker.md`](../docker.md)
- Frontend Design one-click installer: [`docs/install-guide.md`](../../install-guide.md)
- Alibaba Cloud ECS docs: <#>
- Alibaba Cloud ACK docs: <#>
- Alibaba Cloud Container Registry docs: <#>
- ICP filing overview (English): <#>
- ICP filing console (Chinese): <#
