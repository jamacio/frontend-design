# Frontend Design Plugin Registry — Plan (living)

> **One sentence:** Turn the existing `frontend-design-marketplace.json` federation
> into a real npm-/clawhub-/#-style **registry**: GitHub repo as the v1
...
> up their own Frontend Design plugin source with one config line.
...
- [ ] **R3. `SKILL.md` floor stays portable.** A plugin published to OD's
  registry must still install cleanly as a plain agent skill in AI assistant
  Code / code editor / code agent / AI assistant CLI / OpenClaw / Hermes. `frontend-design.json`
...
```text
Plugin source repo
  frontend-design.json includes plugin.repo
        |
...
Plugin artifact
  GitHub ref, GitHub Release .tgz, HTTPS archive, or local folder
        ...
        v
Registry index
  v1: frontend-design/plugin-registry or this repo
      community/**/frontend-design.json
      generated frontend-design-marketplace.json
...
- **#/plugins** is the public renderer of the official and
...
Default community registry
  ...
  -> ~/.frontend-design/plugins/<plugin-id> -> Installed -> agent
```
...
root (`~/.frontend-design/plugins/<plugin-id>` by default). Daemon startup reloads
...
the user describe the plugin, writes `SKILL.md` and `frontend-design.json`, adds
...
entries plus a generated `frontend-design-marketplace.json`. The JSON is what
daemon/CLI/UI fetch; the source entries are what humans review in PRs. This can
start in the main Frontend Design repo, but the code path must still be expressed as
`RegistryBackend` so moving to `frontend-design/plugin-registry` or a database later
...
A dedicated public repo — proposed **`frontend-design/plugin-registry`** — owns the
canonical official catalog. Third parties fork the same shape and point their
own `marketplace.json` URL at it.

```text
frontend-design/plugin-registry/
├── plugins/
│   └── <vendor>/<plugin-name>/
│       ├── manifest.json              ← latest copy of frontend-design.json
│       ├── versions/
...
├── schema/
│   └── frontend-design.marketplace.v1.json
├── .github/workflows/
...
manifest must include `plugin.repo` in `frontend-design.json`, pointing to the
...
4. Fork (or reuse fork of) `frontend-design/plugin-registry` via `gh repo fork`.
...
9. On merge, `publish-index.yml` regenerates `marketplace.json` and pushes
   ...
   JSON, while `#` renders the public browser
...
- `od plugin login` wraps `gh auth login` with Frontend Design copy, scopes, and
...
- **Official site (#/plugins)** — static, SSG against
  repo-owned `plugins/registry/*/frontend-design-marketplace.json` sources. Browse,
  ...
  canonical SEO pages. `#/marketplace` can be kept as an alias
...
- **Plugin detail drawer:** provenance line ("from official · trusted ·
  example/pkg@v0.2.0 · sha256:abcd…"), permissions, README, install
...
- The packaged daemon seeds built-in `official` and `community` registry
  sources from `plugins/registry/*/frontend-design-marketplace.json`. `official` is
...
- Bundled official plugins now carry `sourceMarketplaceId=official` and
  `sourceMarketplaceEntryName=frontend-design/<plugin-id>`, so they are modeled as
...
- `od plugin login` and `od plugin whoami` now delegate to `gh`, and
  registry publishing now has three paths: `--to frontend-design` produces the
...
- `apps/landing-page` now exposes the public SEO renderer at `/plugins/` plus
  ...
  so # can show the ecosystem without calling daemon APIs.
...
- Registry evaluation cases now live in
  ...
  The first covered set locks raw `frontend-design-marketplace.json` source input,
...
- [x] **P1.2 GitHub backend module.** `apps/daemon/src/registry/github-backend.ts`
  implements `RegistryBackend` against `frontend-design/plugin-registry`. Uses
...
- [x] **P2.4 Agent-assisted Create plugin flow.** The `Create plugin` action
  ...
  writes `SKILL.md`/`frontend-design.json`, validates, installs a local test copy,
...
- [x] **P3.1 Stand up `frontend-design/plugin-registry` repo shape.** Schema, validation
...
- [x] **P3.2 Static site renderer.** `apps/landing-page` now
  statically generates `#/plugins` and per-plugin detail routes
  from `plugins/registry/*/frontend-design-marketplace.json` plus bundled official
...
- [x] **P3.5 `od plugin publish --to marketplace-json`.** Lets third-party
  ...
  writing/upserting their own static `frontend-design-marketplace.json`.
...
- [x] **P4.1 DB-backed RegistryBackend.** Same interface, SQLite or Postgres.
  Validates R2.
- [x] **P4.2 Search index.** Static `#/plugins/search.json`
...
2. **Plugin source-of-truth repo.** The source repo can be any shape that
   ...
   requires a `plugin.repo` field in `frontend-design.json` pointing to source.
...
- [ ] `R1`–`R6` invariants each have a regression test that fails when
  violated.
- [ ] A third party can fork `frontend-design/plugin-registry`, change two
...
- [ ] One real third-party publisher (target: a AI assistant-skills-community
