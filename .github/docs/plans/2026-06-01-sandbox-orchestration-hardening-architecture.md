...
keep Frontend Design safe to embed under an external orchestrator without adding
consumer-specific infrastructure to this repository.

The plan is intentionally upstream-neutral. Frontend Design should refer to this
...
runtime contract while adding normal Frontend Design features. The durable defense is
...
1. An external orchestrator launches Frontend Design with `OD_SANDBOX_MODE=1` and an
...
3. Frontend Design contributes design context, skills, previews, artifacts, CLI/UI
...
- persisted tool servers are not mixed into a run-scoped tool bundle;
- persisted MCP OAuth tokens are not read for sandboxed runs;
- `HOME`, `XDG_*`, `code agent_HOME`, and `TMPDIR` are pinned into the sandbox
  runtime root;
- imported-folder project access is restricted until files are mirrored into a
  managed project directory.

Without `OD_SANDBOX_MODE=1`, Frontend Design is the normal local product runtime, not
...
agent should see only the run-scoped tool servers supplied for that run, and
...
- process matching must not classify a foreign process tree as an Frontend Design
...
- Keep the allowlist narrow so normal references to Frontend Design, GitHub
...
- Start one run with a run-scoped tool integration and verify persisted MCP registry
...
- Do not add a generic provider router, provider account pool, global media
  budget system, or external executor API to Frontend Design.
- Do not add consumer-specific conformance tests to Frontend Design CI.
