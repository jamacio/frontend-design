# Frontend Design Plugin & Marketplace — Implementation Plan (living)
...
packages/plugin-runtime/             ← pure TS; reusable in web / daemon / CI
  ...
  ├── adapters/{agent-skill,AI assistant-plugin}.ts
  ├── merge.ts                       ← sidecar + adapter merge; frontend-design.json wins
...
- `packages/plugin-runtime` does not import `node:fs`. It receives `loader: (relpath) => Promise<string>`. Daemon injects real FS, CI injects test doubles, web preview sandbox injects fetch.
...
- **AG-UI is interoperability, not the product UI runtime.** `packages/agui-adapter` and `GET /api/runs/:runId/agui` are shipped so AI assistantKit / AG-UI clients can consume an OD run. The internal web/desktop UI remains OD-native; adding AI assistantKit itself is only justified for an explicit external embed/demo/client.
...
| `plugins/_official/atoms/<atom>/{SKILL.md,frontend-design.json}` | shipped | Phase 4 / 6 / 7 / 8 — 13 first-party atom plugins (4 implemented + 9 reserved fragments) |
| `plugins/_official/scenarios/<id>/{SKILL.md,frontend-design.json}` | shipped | Phase 4 (§23.3.3) — bundled scenario/router/export plugins, including the four taskKind defaults plus `od-default` Home free-form routing |
...
| `tools/pack/helm/frontend-design/templates/**` | shipped | Phase 5 — Deployment / Service / Secret / ConfigMap / PVCs / Ingress / NOTES |
| `tools/pack/helm/frontend-design/values-{aws,gcp,azure,aliyun,tencent,huawei,self}.yaml` | shipped | Phase 5 — per-cloud overrides (volume + ingress diffs) |
...
| `GET /api/runs/:runId/agui` | shipped | Phase 4 — pipes events through `@frontend-design/agui-adapter` |
...
| `od plugin export <projectId> --as od\|AI assistant-plugin\|agent-skill` | shipped | Phase 4 — `apps/daemon/src/plugins/export.ts` + `POST /api/applied-plugins/export` |
| `od plugin publish --to AIs-skills\|awesome-agent-skills\|clawhub\|skills-sh` | shipped | Phase 4 — `apps/daemon/src/plugins/publish.ts` + `--open` browser launch |
...
- [x] `docs/schemas/frontend-design.plugin.v1.json` — JSON Schema v1.
- [x] `docs/schemas/frontend-design.marketplace.v1.json` — JSON Schema v1.
...
- [x] `pnpm --filter @frontend-design/plugin-runtime test`
- [x] `pnpm guard && pnpm typecheck`
- [x] CI digest stability: re-running `digest()` on the fixtures matches the pinned hex.

Exit criterion

- Importing `import type { ApplyResult, AppliedPluginSnapshot } from '@frontend-design/contracts'` works from daemon and web. ✓ verified.
...
- [x] `pnpm --filter @frontend-design/plugin-runtime test` covers: digest stability, `parseManifest` + `parseMarketplace`, SKILL frontmatter adapter, sidecar+adapter merge precedence, `validateSafe` cross-field rules.
...
- [ ] e2e: restricted plugin cannot start tool server until Grant clicked; check `applied_plugin_snapshots.capabilities_granted` updates.
...
- [x] `od plugin export <projectId> --as od|AI assistant-plugin|agent-skill` — `apps/daemon/src/plugins/export.ts` + `POST /api/applied-plugins/export`.
...
- [x] `od plugin publish --to AIs-skills|awesome-agent-skills|clawhub|skills-sh` (PR template launcher) — `apps/daemon/src/plugins/publish.ts`.
...
- [x] `@frontend-design/agui-adapter` package; `GET /api/runs/:runId/agui` SSE endpoint emits AG-UI canonical events.
...
- [ ] AG-UI smoke: a AI assistantKit React client subscribes to `/api/runs/:runId/agui` and renders surfaces unmodified. This is an external-interop smoke, not a blocker for OD-native web/desktop rendering.
...
- [x] `pnpm --filter @frontend-design/contracts test` clean.
- [x] `pnpm --filter @frontend-design/plugin-runtime test` clean.
- [x] `pnpm --filter @frontend-design/daemon test` — all 56 `plugins-*.test.ts` (391 tests) green. Three unrelated pre-existing failures remain (`finalize-design.test.ts` resolveCurrentArtifact path normalization, `chat-route.test.ts` stalled-json-stream timeout, `connection-test.test.ts` hard-cancel timeout). They were inherited from PR #832 and the chat/connection timeout test refactors and do not block the plugin loop; tracked separately.
- [x] `pnpm --filter @frontend-design/web test` clean.
