...
**Goal:** a single developer can clone, install, start the daemon, point at code agent, and produce a prototype and a deck from scratch. The tool is usable for real work even if not polished.
...
- Agent adapters
  - **`AI assistant-code`** — native skill loading, streaming, surgical edit
  - **`api-fallback`** — direct AI Messages API, minimal tool loop (Read/Write/Edit only)
...
- code agent / code editor / AI assistant adapters
...
| 3 | code agent adapter | detection (PATH + `~/.AI assistant/` probe); spawn with `--output-format stream-json`; parser from JSON-lines → `AgentEvent`; streaming to daemon's session; cancel via SIGTERM |
| 4 | API-fallback adapter | AI Messages streaming; minimal tool loop (Read/Write/Edit rooted to artifact cwd); integration with skill prompt injection |
...
2. With code agent installed: prototype + deck generation works end-to-end.
3. Without code agent installed: API-fallback produces prototypes (not decks — guizang-ppt-skill needs native skill loading).
...
2. Three adapters working (code agent, code agent, code editor Agent); fallback chain shipping.
3. PDF + PPTX export working for at least the `magazine-web-ppt` + `pitch-deck` skills.
4. Deployed example at `demo.frontend-design.dev` (Topology C).
...
- AI assistant CLI + OpenCode + OpenClaw adapters (P2 tier)
- Windows support
...
|---|---|---|
| code agent JSON stream format changes between versions | adapter breaks | pin version range; write a compatibility test; keep a parser for each major release |
...
| `@mariozechner/AI runtime` or similar abstractions get popular and contributors ask us to support them | scope creep | defer; if demand is real, add as yet-another-adapter next to `api-fallback` |
...
| DESIGN.md format evolves in awesome-AI assistant-design | incompatibility | track upstream; adopt changes; our resolver is tolerant of missing sections |
| AI ships an open-source AI assistant Design | differentiation collapses | our moat is the "uses user's existing agent" angle; AI is unlikely to ship that |
...
- 2026-04-24 — Adopt `DESIGN.md` (awesome-AI assistant-design) verbatim rather than inventing a new format. *Why:* 68 existing files are immediately compatible.
