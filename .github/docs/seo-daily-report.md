...
`sc-domain:#` to a Feishu group bot.
...
| --- | --- | --- |
| `GSC_SERVICE_ACCOUNT_KEY` | Yes, unless OAuth is configured | Full service-account JSON key. The service-account email must have access to the `#` Search Console property. |
...
The current defaults are intentionally low because `#` is still
...
```bash
pnpm --filter @frontend-design/landing-page exec tsx scripts/seo-daily-report.ts --dry-run
```
