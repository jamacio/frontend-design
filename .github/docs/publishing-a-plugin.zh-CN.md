# 发布 Frontend Design 插件

Frontend Design registry v1 复用 GitHub 作为后端。CLI 是 canonical workflow；
...
公开 registry ID 必须是 `vendor/plugin-name`。生成的 `frontend-design.json`
...
只需要 `SKILL.md` 和 `frontend-design.json`。
...
这两个命令包装 GitHub CLI。token 留在 `gh`，Frontend Design 不保存 GitHub
凭据。

## 4. 发布

```bash
od plugin publish vendor/plugin-name --to frontend-design --repo 
```

v1 会打开 GitHub registry review flow。发布 payload 包含插件 ID、版本、
源码仓库、能力摘要、包 digest 和 registry entry path。合并之后，CI 重新生成
`frontend-design-marketplace.json`。
