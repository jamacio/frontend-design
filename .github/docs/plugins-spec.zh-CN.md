# Frontend Design 插件与 Marketplace 规范（v1）

> **一句话总结：** Frontend Design 插件把可移植的 `SKILL.md` 能力包装成 marketplace 可发现、一键可用的设计工作流，同时保留对现有 agent skill 生态、headless CLI 使用方式和自托管部署的兼容性。
...
**Plugin（插件）** 是 Frontend Design 的分发单元。[Skill](skills-protocol.md) 描述的是 agent 可以执行的一项能力，而 Plugin 是围绕这项能力形成的可发布包：一个或多个 skills、可选 design system 引用、可选 craft 规则、可选 AI assistant-plugin 资产、预览、use-case query、资产文件夹，以及一个用于驱动 OD marketplace 表面的轻量机器可读 sidecar。插件始终以可移植的 `SKILL.md` 为锚点，因此可以不经修改地发布到现有 agent skill 生态。

> **兼容性承诺（扩展 [`skills-protocol.md`](skills-protocol.md)）：** 任何包含 `SKILL.md` 的插件文件夹，都可以作为普通 agent skill 在 code agent、code editor、code agent、AI assistant CLI、OpenClaw、Hermes 等工具中运行。添加 `frontend-design.json` 只是纯增量能力：它会解锁 OD 的 marketplace 卡片、预览、一键「使用」流程、类型化 context-chip strip，但不会改变底层 skill 的运行方式。**一个 repo，两种消费模式。**
...
1. **插件作者发布可移植能力。** `SKILL.md` 仍然是可执行的 agent contract；`frontend-design.json` 增加 OD marketplace 元数据、输入字段、默认值、预览和上下文 wiring。
...
| 问题 | Figma 时代插件假设 | Frontend Design v1 的答案 |
| --- | --- | --- |
...
| Headless CLI / code agent | code agent、code editor、code agent、CI 或脚本调用 `od` | `od run create --plugin make-a-deck --input audience=investors --input topic='AI design ops'` | 不打开 desktop，也得到同一套 manifest resolution、context chips 和 run events。 |
...
6. [`frontend-design-marketplace.json` schema](#6-frontend-design-marketplacejson--联邦目录)
7. [发现与安装](#7-发现与安装)
8. [Apply pipeline](#8-apply-pipeline)
9. [信任与能力](#9-信任与能力)
10. [一方 atoms](#10-一方-atoms--frontend-design-的原子能力)
11. [架构：现有 repo 要改什么](#11-架构现有-repo-要改什么)
12. [CLI 表面](#12-cli-表面)
13. [公网 Web 表面](#13-公网-web-表面-frontend-designaimarketplace)
14. [发布与目录分发](#14-发布与目录分发)
15. [部署与可移植性：Docker，任意云](#15-部署与可移植性docker任意云)
16. [分阶段实现计划](#16-分阶段实现计划)
17. [示例](#17-示例)
18. [风险与开放问题](#18-风险与开放问题)
19. [为什么这是 Frontend Design 的重要一步](#19-为什么这是-frontend-design-的重要一步)
...
- 写作阶段：草稿，等待评审。
- 规划阶段锁定的默认选择（评审时可覆盖）：
  - **兼容性 = wrap-then-extend。** 现有 `SKILL.md` 与 `.AI assistant-plugin/plugin.json` repo 原样可运行；`frontend-design.json` 是增量 sidecar。
...
Frontend Design 变成一套 **server + CLI + atomic core engine + plugin/marketplace system**。产品表面发生反转：不再是「点一个按钮，填一个表单」，而是用户打开 marketplace，点击某个插件，输入框自动填入 query，并在上方注入类型化 context chips。相同的插件文件夹也同时是 code agent、code editor、code agent、AI assistant CLI、OpenClaw、Hermes 可消费的 agent skill，并且可以作为独立 GitHub repo 发布到：
...
不同目录的收录格式不同，但它们都索引 `SKILL.md` 形态的文件夹。只要保持 `SKILL.md` 作为 canonical，`frontend-design.json` 作为严格 sidecar，一个 repo 就可以不做目标目录专用改写而进入所有生态目录。

同一愿景的第二条轴线：**CLI 是 Frontend Design 面向 agent 的 canonical API。** 代码 agent（code agent、code editor、code agent、OpenClaw、Hermes、企业内部 orchestrator）通过 shell 调用 `od …` 驱动 OD，而不是直接请求 `/api/*`。CLI 用稳定的子命令 contract 包装所有 server 能力：project 创建、conversation/run 生命周期、plugin apply、project 文件系统操作、design library introspection、daemon control。HTTP server 是 desktop UI 与 CLI 自身的实现细节；agent 如果直接访问 HTTP，就绕过了 contract。

第三条轴线来自第二条：**OD 可以完全 headless 运行；UI 是效率层，而不是运行时依赖。** 用户只有 code agent（或 code editor、code agent、AI assistant CLI）和已安装的 `od`，也能浏览 marketplace、安装插件、创建 project、拉起任务、消费产物，全流程不需要启动 desktop app。OD desktop UI 的价值类似 code editor IDE 相对于 `code editor-agent` CLI：更快发现、实时 artifact preview、chat/canvas 并排、marketplace 浏览、direction-picker GUI、critique-theater 面板。这些都是同一批 primitives 之上的体验增强。每个 UI 功能都必须先能表达为 CLI 子命令或 streaming event；UI 消费这些 primitives 并添加呈现层。这个解耦由架构规则强制（§11.7）。
...
1. 每个可运行、可分发的 OD 插件都是合法 agent skill（以 `SKILL.md` 或 `.AI assistant-plugin/plugin.json` 为锚点）。不 fork skill spec。
2. 一个普通 skill 或 AI assistant-plugin repo，只要添加可选 `frontend-design.json` sidecar，就成为 OD 插件；不改名、不改 body。
3. 支持四类安装源：本地文件夹、GitHub repo（可带 ref/subpath）、任意 HTTPS archive，以及联邦 `frontend-design-marketplace.json` index。
4. 一键「使用」会自动填充 brief 输入框，并在上方填充 `ContextItem` chips（skills、design-system、craft、assets、MCP、AI assistant-plugin、atom）。
5. 默认分层信任；能力 scope 是声明式、可选的。
6. OD core engine、atomic capabilities、plugin runtime 全部可以通过 CLI 访问，因此任何 code agent 都能 headless 地驱动 Frontend Design。
...
- 替代 SKILL.md / AI assistant-plugin spec：OD 永不 fork。
- 托管插件二进制：OD 指向 GitHub / CDN URL；存储由发布者负责。
- 签名/PKI 生态：能力 gating 依赖用户授权，而不是签名。
- 一个由 Frontend Design SaaS 代表用户运行 agent 的 web-hosted marketplace：v1 只做 local-first / self-hostable。

## 3. 兼容性矩阵：什么样的文件夹对哪些系统是合法插件

| 存在的文件 | OD 可安装 | code agent / code editor / code agent / AI assistant CLI | OpenClaw / Hermes | awesome-agent-skills | clawhub | # |
| --- | --- | --- | --- | --- | --- | --- |
| 仅 `SKILL.md` | yes | yes | yes | yes | yes | yes |
| 仅 `.AI assistant-plugin/plugin.json` | yes | yes (AI assistant) | partial | listable | listable | listable |
| 仅 `frontend-design.json` | metadata-only | no | no | no | no | no |
...
结论：**`SKILL.md` 是最低共同分母**。任何推荐用于分发的插件都应该包含 `SKILL.md`，这样它可以进入所有主流目录；再添加 `frontend-design.json` 来获得 OD 的产品表面。

仅包含 `frontend-design.json` 的文件夹在 v1 中不是可运行插件，而是 **metadata-only preset**：OD 可以读取它来展示市场卡片、聚合远端引用或作为未来 install stub，但它不能触发 agent run，也不能进入跨 agent catalog。`od plugin doctor` 必须把这种形态标记为 `metadata-only`，并提示作者补充 `SKILL.md` 或 `.AI assistant-plugin/plugin.json` 才能发布为 runnable plugin。
...
- `frontend-design.json` 只**指向** SKILL.md / DESIGN.md / craft 文件；永不复制它们的正文。
- 当前 SKILL.md frontmatter 上已有的 OD 专属 `od:` namespace（已在 [`skills-protocol.md`](skills-protocol.md) 中描述，并在 [`skills/blog-post/SKILL.md`](../skills/blog-post/SKILL.md) 中使用）继续作为没有 `frontend-design.json` 的插件的 fallback。我们不废弃它，只在其上叠加。
- v1 的 runnable plugin 必须至少包含 `SKILL.md` 或 `.AI assistant-plugin/plugin.json` 之一。`frontend-design.json` 本身不定义 agent 行为，只定义 OD 如何展示、解析和应用这些行为。
...
- `specVersion`：解释此 manifest 时使用的 Frontend Design 插件规范版本。它独立于插件 `version`，并会冻结到 apply snapshot，便于 replay。
...
| `mcp` | daemon 在 run start 阶段写 `.mcp.json`，使插件里的 tool servers 启动 |
| `subprocess` / `bash` | 执行 AI assistant-plugin hooks，开启 agent-callable bash tools |
...
- 声明 `.AI assistant-plugin` hooks 时，隐含需要 `subprocess`；hook 如果读取 bundled assets，还需要 `fs:read`。
...
- provider credentials 不是 v1 capability；插件不能直接声明访问 `AI_API_KEY`、media provider keys 或 connector secrets。需要 credentials 的能力必须通过 OD 自己的 first-party atom / tool 间接调用。

### 5.4 `SKILL.md` frontmatter 到 `PluginManifest` 的映射

当插件没有 `frontend-design.json`，但 `SKILL.md` 已经包含 [`skills-protocol.md`](skills-protocol.md) 中定义的 `od:` frontmatter 时，`adapters/agent-skill.ts` 会合成一个最小 `PluginManifest`。映射规则必须稳定，避免旧 skill 与新 plugin schema 出现两套语义：
...
如果 `frontend-design.json` 与 `SKILL.md` frontmatter 同时存在，`frontend-design.json` 优先，但 loader 必须保留 adapter 产生的 warnings。这样作者可以渐进迁移：先让旧 skill 原样可用，再逐步增加 OD marketplace 信息。

## 6. `frontend-design-marketplace.json` — 联邦目录

它镜像 [`AIs/skills/.AI assistant-plugin/marketplace.json`](# assistant-plugin/marketplace.json) 的形态，因此现有社区 catalog 只需要重命名即可复用。
...
Marketplace 顶层 `version` 是 catalog snapshot 版本；每个 `plugins[]` entry 也声明被列入的插件版本。Installer 抓取后仍会校验目标文件夹自己的 `frontend-design.json`，但 registry search、审计日志和 marketplace refresh events 可以在安装前就理解 catalog 与插件版本。
...
| Priority | 路径 | 资源形态 | 来源 |
| --- | --- | --- | --- |
| 1 | `<projectCwd>/.frontend-design/plugins/<id>/` | plugin bundle | 新增，与用户代码一起提交；必须显式安装到 project |
| 2 | `<projectCwd>/.AI assistant/skills/<id>/` | legacy `SKILL.md` | 沿用 [`skills-protocol.md`](skills-protocol.md) 的 project-private skill 兼容路径 |
| 3 | `<daemonDataDir>/plugins/<id>/` | plugin bundle | 新增，由 `od plugin install` 写入 daemon data root |
| 4 | `~/.frontend-design/skills/<id>/` | legacy `SKILL.md` | OD canonical skill install path；可 symlink 到其它 agent |
| 5 | `~/.AI assistant/skills/<id>/` | legacy `SKILL.md` | 外部 code agent / skills 工具写入的兼容路径，只读扫描 |
| 6 | repo root `skills/`, `design-systems/`, `craft/` | bundled resources | 现有一方资源，不变 |

冲突解决：按 normalized `name` / plugin id；数字越小优先级越高。legacy `SKILL.md` location 被 adapter 合成为 plugin record，但不会被复制到 `<daemonDataDir>/plugins/`，除非用户显式执行 `od plugin install`。这样现有 AI assistant skills 继续零配置可用，新 plugin bundle 也有清晰的安装根。
...
GitHub install 使用 `codeload install path`，不需要 git binary，同时包含 path-traversal guard 和可配置 size cap。
...
export interface AppliedPluginSnapshot {
  ...
  mcpServers: McpServerSpec[];                  // apply 时启用的 tool server set；与 mcpServers 字段同义但冻结到 snapshot
}
...
`GET /api/atoms` 返回 atoms 与已知 reference pipelines。当前实现已经开始自举：一方 atom plugins 位于 `plugins/_official/atoms/**`，bundled scenario plugins 位于 `plugins/_official/scenarios/**`，`renderActiveStageBlock(stageId, bodies)` 会把 active stage 的 atom bodies 注入 prompt。因此 system prompt 现在已经 pipeline-aware，但还不是完全 data-driven：Frontend Design 基础 designer prompt、discovery philosophy 和部分入口默认逻辑仍在 daemon / product code 中。这足以支撑"插件组装核心管线"这条主张，但不假装所有行为字节都已经迁到插件。

### 10.3 Generative UI：AG-UI–inspired surfaces

OD 接受 [AI assistantKit / AG-UI 协议] 中有价值的部分：agent 可以在 run 中请求交互 UI。OD **不**允许 agent 在主产品表面自由发明 app UI 或视觉样式。v1 提供自己的 `GenUISurface*` discriminated union，跟现有 `PersistedAgentEvent`、SSE / ND-JSON 流共用通道；`@frontend-design/agui-adapter` 会把这些事件投影成 AG-UI canonical events 供外部 client 使用。
...
export interface GenUISurfaceSpec {
  ...
  oauth?: {
    ...
    mcpServerId?: string;                      // route='mcp' 时必填，引用 plugin 自带 tool server 的 name
  };
}
```
...
| `mcp` | 复用 `POST /api/mcp/oauth/start` flow，token 落到 `<dataDir>/mcp-tokens.json` | 沿用 Settings → tool servers 的 OAuth 视觉 | `genui_surfaces.value_json = { mcpServerId }`；token 不进 SQLite |
| `plugin`（Phase 4） | plugin 提供任意 third-party OAuth metadata；daemon 走通用 PKCE adapter | TBD | TBD |

`od plugin doctor` 在 install / apply 阶段强制：(1) `oauth.route === 'connector'` 时，`oauth.connectorId` 必须出现在同 plugin 的 `od.connectors.required[]` 或 `od.connectors.optional[]` 中；(2) `oauth.route === 'mcp'` 时，`oauth.mcpServerId` 必须匹配 plugin 自带的某个 tool server name。
...
| Frontend SDK 兼容 | OD desktop / web 自带 renderer | `@frontend-design/agui-adapter` 让 AI assistantKit / 其他 AG-UI client 直接消费 OD run |

Adapter 是互操作表面，不是内部 UI 的 source of truth。除非另有外部 embed / demo / client 明确需要，否则 OD 不应把 AI assistantKit 加成主产品依赖。v1 plugin **不需要改**就可以在 AG-UI ecosystem 内被消费，因为 adapter 是 OD 自有事件的一层投影。
...
- `parsers/manifest.ts`：读取 `frontend-design.json` → `PluginManifest`（Zod-validated）。
- `adapters/agent-skill.ts`：读取 `SKILL.md` → 基于 [`skills-protocol.md`](skills-protocol.md) 里的 `od:` frontmatter 合成 `PluginManifest`。
- `adapters/AI assistant-plugin.ts`：读取 `.AI assistant-plugin/plugin.json` → 合成 `PluginManifest`。
- `merge.ts`：合并 sidecar + adapters，`frontend-design.json` 优先；foreign content 落到 `compat.*`。
...
CREATE TABLE applied_plugin_snapshots (
  ...
  mcp_servers_json        TEXT NOT NULL DEFAULT '[]',  -- apply 时启用的 tool server set 冻结视图（McpServerSpec[]）
...
> **Transport equivalence（经验规则）。** 上面每个 endpoint 都同时暴露为 CLI subcommand，并在适合 MCP 语义的地方暴露为 tool integration。代码 agent 应该使用 CLI；只有 desktop web app 和 `od …` 自己直接使用 HTTP。完整 CLI 表面见 §12。
...
OD 运行在三种 operating modes，它们共享**同一个** daemon、**同一个** CLI、**同一个** plugin runtime。差异只是呈现方式。这与 code editor 的形态一致：`code editor-agent`（CLI）自身足够运行；IDE 是增强层。

| Mode | 运行内容 | 适用场景 | 入口 |
| --- | --- | --- | --- |
| **Headless** | 仅 daemon process；无 web bundle、无 electron | CI、server、container、AI assistant-Code-driven flows | `od daemon start --headless`（Phase 2 新 flag） |
...
- 用户只有 **code agent**（或任意 code agent）加 `npm i -g @frontend-design/cli`，再启动一个 headless daemon，就能完成 install plugin → create project → run → consume artifacts 全流程。不需要 OD desktop。
...
- CI 是一等公民：GitHub Action 可以 `npm i -g @frontend-design/cli && od daemon start --headless && od plugin install … && od run start --project … --follow`。无 display、无 electron、无 UI scripting。
...
| MCP stdio | MCP-aware agents（code agent、code editor 等） | `od mcp` 与 `od mcp live-artifacts`（现有） |

当新能力发布时，CLI subcommand 是 primary contract。HTTP route 用于支撑 CLI；tool server 把一组精选 CLI subcommands 暴露为 tools。版本策略：subcommand names、argument names、`--json` schemas 由 `packages/contracts` 管理并在 CI 中测试；breaking changes 跟随 `od` bin 的 major-version bump。
...
一个通过 CLI 驱动 Frontend Design 的 code agent 通常这样做：
...
## 13. 公网 Web 表面（#/marketplace）

产品网站已经是 [#](#)。公网 marketplace 作为同一站点下的路径发布：`#/marketplace`（canonical），并把 `#/plugins` 作为 alias；不是单独域名。它是一个从官方 `frontend-design-marketplace.json` index 渲染出的 static-rendered catalog，插件详情页由每个 listed repo 内的同一份 `frontend-design.json` 支撑。视觉上它类似 [`#`](#) 对 skills 的处理，但详情页会渲染 OD 专属 previews（`od.preview.entry` HTML、sample outputs、use-case query、chip preview）。

这个站点与 in-app marketplace 共享一个 source of truth：

- 同一组 JSON Schemas（`#`、`#`）。
- 同一套联邦 listing format（`frontend-design-marketplace.json`）。
- 同一份 plugin manifests（每个 repo 内的 `frontend-design.json`）。
...
| Public marketplace（`#/marketplace`） | 匿名访客、SEO、分享 | Deep-link `od://plugins/<id>?apply=1`（自动在 desktop app 中安装并 apply），以及 “Copy install command” |
...
desktop app 注册 `od://` URL scheme；点击 `#/marketplace` 上的按钮时，如果 desktop 已安装则启动 desktop，否则 fallback 到「How to install Frontend Design」流程。
...
| --- | --- | --- |
| [`AIs/skills`] | 带 `name`/`description` frontmatter 的 `SKILL.md` | `SKILL.md`（canonical anchor） |
| [`AIs/AI assistant-code/plugins`] | `.AI assistant-plugin/plugin.json` + standard structure | `.AI assistant-plugin/plugin.json`（可选） |
| [`VoltAgent/awesome-agent-skills`] | PR 增加一行指向 repo URL | repo URL，§14.1 中的 automation 辅助 |
| [`openclaw/clawhub`] | 通过 clawhub web app 或 PR 提交 | repo URL |
| [`#`](#) | 一旦观察到 `npx skills add owner/repo` 即可被索引 | repo URL |
| `frontend-design-marketplace.json` | entry 指向 `github:owner/repo` | `frontend-design.json` 丰富 listing |

### 14.1 作者工具

- `od plugin scaffold`：写入 starter folder，包含 `SKILL.md`（行业标准，带 `od:` frontmatter 以向后兼容）和 `frontend-design.json`（OD enrichment，`compat.agentSkills` 指向 SKILL.md）。
- `od plugin doctor`：运行 JSON Schema、SKILL.md frontmatter parser，以及「它看起来是否能被 awesome-agent-skills / clawhub / # 收录？」lint，检查 README、license file、frontmatter 完整性。
...
1. **Skill-only consumption（完全没有 OD）。** code editor 用户执行 `npx skills add frontend-design/make-a-deck`。code editor 读取 `SKILL.md` 并运行 workflow。没有 OD CLI、没有 OD daemon。插件的 marketplace polish（`frontend-design.json`）被忽略，code editor 看到的是普通 skill。
2. **Headless OD（CLI + code agent，无 OD UI）。** 高阶用户继续使用自己偏好的 code agent：code agent、code editor、code agent 等，但把 OD 作为 side service 加进来，获得 plugin context resolution、project bookkeeping、design library injection、artifact tracking。无 browser、无 electron。具体 pipeline 见 §14.3。
...
### 14.3 具体 headless pipeline（code agent + `od` CLI，无 OD UI）

这与 code editor-agent + scripts 对 code editor 的意义相同：code agent 做思考，OD CLI 提供 project / plugin / artifact substrate。
...
### 14.4 类比：code editor vs `code editor-agent`，OD desktop vs `od` CLI

心智模型：

| Layer | code editor | Frontend Design |
| --- | --- | --- |
| Headless agent CLI | `code editor-agent`（驱动 agent loop） | `od run start --agent AI assistant --follow` + `od plugin run` |
| Local services / db | code editor 的 background indexing / state | OD daemon、SQLite、`.od/projects/<id>/` |
...
- **Tag**：`ghcr.io/frontend-design/od:<version>`，以及 moving `:latest` 与 `:edge`。
...
- **Contents**：
  ...
  - OD 支持作为 agent backends 的 bundled code-agent CLIs：code agent、code agent CLI、AI assistant CLI。每次 run 可选；默认是 `OD_AGENT_BACKEND`。
...
- 新 `tools/pack/helm/`：Helm chart，values presets 用于每个云的 volume + secret patterns。chart 保持 generic；cloud-specific bootstrap（CloudFormation / Deployment Manager / ARM / Aliyun ROS / Tencent TIC / Huawei RFS）放在独立 `frontend-design/deploy` repo 中，允许单独演进。
...
1. **Self-hosted enterprise。** 企业托管私有 OD instance，注册内部 `frontend-design-marketplace.json`（`od marketplace add https://internal/...`），限制插件只来自内部审过的集合。设计师和 PM 本地使用 desktop client；CI 使用 `docker exec od od …`。
...
- JSON Schemas：`docs/schemas/frontend-design.plugin.v1.json` 与 `frontend-design.marketplace.v1.json`。
...
> **设计原则：** Phase 1 完成时，外部 code agent（code agent、code editor、code agent、CI 脚本）必须能**完整**走完 `install plugin → create project → run → consume artifact` 闭环，不依赖任何 UI。spec 强调 CLI 是 canonical agent-facing API（§12），所以最小 CLI 闭环必须与 plugin verbs 一起在 Phase 1 落地，不能拖到 Phase 2C。
...
- `pnpm --filter @frontend-design/plugin-runtime test`（parser fixtures：pure SKILL.md、pure AI assistant plugin、metadata-only frontend-design.json、三者组合、SKILL frontmatter mapping）。
...
Validation：从本地 mock marketplace.json 安装 plugin、rotate ref、uninstall。Restricted plugin 在点击 "Grant" 前不能启动 tool server。

### Phase 4 — Atoms, publish-back, full CLI parity（1–2 周，可拆）

- 在 `docs/atoms.md` 中记录 atoms；通过 `GET /api/atoms` 暴露。
- `od plugin export <projectId> --as od|AI assistant-plugin|agent-skill`：从已有 project 生成 publish-ready folder。
- `od plugin run <id> --input k=v --follow`：apply + run start + watch 的 shorthand wrapper。
- `od plugin scaffold` interactive starter。
- `od plugin publish --to AIs-skills|awesome-agent-skills|clawhub` 打开 PR template。
...
- **§10.3.5 AG-UI 完整对齐：**
  - 新 package `@frontend-design/agui-adapter`：把 OD 的 `PersistedAgentEvent` + `GenUIEvent` 双向映射到 AG-UI canonical events（`agent.message`、`tool_call`、`state_update`、`ui.surface_requested`、`ui.surface_responded`）。
  - daemon 增加可选 `/api/runs/:runId/agui` SSE 端点，输出 AG-UI canonical events，使 AI assistantKit / 其他 AG-UI client 直接消费 OD run。
  ...
  - Open-Ended (MCP-Apps / Open-JSON) 模式：让 plugin 通过 tool server 推送任意 JSON UI tree，desktop / web 以受限 schema 渲染。
...
- **Per-cloud one-click templates** 放在独立 `frontend-design/deploy` repo（CloudFormation、Deployment Manager、ARM、Aliyun ROS、Tencent TIC、Huawei RFS），非阻塞，单独跟踪。
...
OD 通过 [`skills-protocol.md`](skills-protocol.md) 中现有 `od:` frontmatter loader 把它当作插件读取。不需要 `frontend-design.json`：这个插件缺少 marketplace polish，但完全可运行。
...
`SKILL.md` 保持可移植：code editor / code agent / OpenClaw 可直接读取。`frontend-design.json` 增加 preview、query、chip strip、capabilities。这个 repo 可以不经修改地列入 §14 的所有 catalog。
...
| Risk | Mitigation |
| --- | --- |
| OD plugin schema 与更广义 skill spec 发生 drift | `frontend-design.json` 仅为 sidecar；永不修改 SKILL.md。CI against public AIs/skills repo。 |
...
| Project-local plugins 被提交到用户 repo | 仅发现 `<projectCwd>/.frontend-design/plugins/`；通过 `od plugin install --project` opt-in。 |
| Trust model 让 community plugins 默认半功能 | 详情页提供清晰 capability checklist 与一键「Grant all」；restricted 行为显式，不静默。 |
| 插件自带 tool servers 可能无法启动 | `od plugin doctor` dry-launch declared MCP commands；在「Use」前暴露失败。 |
...
| Multi-cloud testing matrix 太大 | Phase 5 先发布一个 canonical compose smoke（单云），再逐步加云；per-cloud one-click templates 放在 `frontend-design/deploy` 独立演进（§15.5）。 |
...
| AG-UI 协议生态可能演进，OD 自有 wire-format 与 AG-UI canonical 漂移 | OD-native `GenUIEvent` 仍是内部 source of truth；`@frontend-design/agui-adapter` 是外部投影层，因此 upstream 协议升级不绑死 daemon 或 web renderer release cadence。 |
...
- **Marketplace JSON shape**：是否偏离 AI 的 `marketplace.json` shape，还是保持 byte-compatible，从而复用现有 AI assistant-plugin marketplaces？（默认：保持 byte-compatible。）
...
- ~~**AG-UI 协议引入时机**~~：**已解决。** `@frontend-design/agui-adapter` 与 `GET /api/runs/:runId/agui` 已作为可选互操作表面交付；OD-native GenUI 仍是内部 renderer，AI assistantKit 不是主产品必需依赖。

## 19. 为什么这是 Frontend Design 的重要一步

- **继承供给。** `AIs/skills`、`awesome-agent-skills`、`clawhub`、`#` 上的每个 public agent skill，只需一个可选 `frontend-design.json` 就能成为 OD 插件；反过来，每个 OD 插件也能不经修改发布到这些 catalog。
...
5. **CLI-first headless 模式**（§11.7，§14.3）。这是 v1 阶段场景 4 的 fallback：OD 把 SKILL.md / DESIGN.md / craft / 生成 artifact 都 stage 到 project cwd，由 code editor / code agent / code agent 在那个 cwd 内完成 patch。这条接力在 §14.3 已规定；§21.5 把它升格为 v1 的"设计→生产代码"明确契约。
...
**v1 fallback（同时见 §21.5）：** 不在 OD 内部端到端跑这条，而是引导用户走 §14.3 的 headless pipeline —— OD 产出 `html-prototype` 或 `implementation-plan` artifact；code editor / code agent 在用户自己的 repo cwd 里 apply patch 并跑 build/test。v1 不阻挡这条路径，只是不拥有 patch-application 这一步。
...
- "设计 → 生产代码" 在 v1 是一个**双产品接力**：OD 拥有 design substrate（SKILL.md / DESIGN.md / craft / 生成 artifact 都被 stage 进 project cwd，加上 `od files` 管理的 artifact 簿记）；用户原本的 code agent（code editor / code agent / code agent / AI assistant CLI）在用户的 repo cwd 内拥有真实 repo patch。
...
2. **用户的 code agent 在该 cwd 或自己的 repo cwd 内操作。** OD 不在 IDE 里跑；它作为 daemon 与 IDE 并列。code editor / code agent / code agent / AI assistant CLI 是 patch-application 的表面。
...
| --- | --- | --- |
| 调一个 OD 没有的工具（Figma REST、AST 解析、SVG 转换…） | 在 `od.context.mcp[]` 里捆绑一个 tool server | §5 / §5.3（`mcp` + `subprocess` + `network`） |
...
总结成一条规则：**OD 一方 atom 缺位 → plugin 作者用 `tool server + bash + SKILL.md` 三件套替代**。代价是 ergonomics（每个 plugin 各自重新发明命名和 prompt fragment），不是能力。
...
- 最后一个 stage 触发一个 `confirmation` GenUI surface："准备 apply 吗？把 project cwd 在 code editor / code agent / code agent 中打开，按附带的指令执行。"
...
2. **atom registry 是封闭的**。`od.pipeline.stages[*].atoms[]` 必须命中 §10 catalog（implemented + reserved）。plugin 作者无法注册新 atom 名字。变通：把新能力封成 tool integration，挂到最近的通用 atom（`file-write` / `todo-write` / `critique-theater`）下；daemon 发出的 `pipeline_stage_started` / `pipeline_stage_completed` 事件流就丢失这一步的粒度。
...
2. **稳定契约**。SKILL.md 指令、tool integration surface、pipeline shape 稳定后，提取成候选一方 atom 契约：atom id、prompt fragment、tool gating、可选的 `until` 信号、可选的 GenUI surface 默认值。
...
| --- | --- | --- |
| **A. 用户态（可移到 plugin）** | 一方 plugin 的 manifest + SKILL.md | atom prompt fragment、默认 reference pipeline、atom 自带的 tool server、critique 评分坐标、discovery 提问措辞 |
...
| **C. v1 已经是 plugin 驱动** | 已经在 `<daemonDataDir>/plugins/...` 或 per-project plugin 文件夹 | 当前 SKILL.md、DESIGN.md、craft .md、plugin 声明的 tool server、plugin 声明的 GenUI surface、plugin 声明的 pipeline |
...
- plugin 声明的 tool server、connector 需求、GenUI surface、`od.pipeline.stages[]` 全部走的是和第三方 plugin 同一套 registry / resolver 路径。
...
- `@frontend-design/agui-adapter` 与 `/api/runs/:runId/agui` 提供外部 AG-UI event projection，同时不改变 OD 内部 GenUI renderer。
...
- atom plugin 是带 `frontend-design.json`（`od.kind: 'atom'`）+ `SKILL.md` 的目录。
- `SKILL.md` 正文就是该 atom 的 prompt fragment；当某个 stage 用 id 引用了该 atom，daemon assembler 把它注入。
- 可选的 `od.context.mcp[]` 声明该 atom 用到的 tool integration（如 `live-artifact`、`connector`）。
...
- `plugins/_official/scenarios/od-figma-migration/frontend-design.json`（Phase 6 后）
- `plugins/_official/scenarios/od-code-migration/frontend-design.json`（Phase 7 后）
