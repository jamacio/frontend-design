...
| SET-006 | 历史 AI provider-compatible 已知 provider 切到 AI 时，会命中对应 sibling preset | `settings-api-protocol.test.ts`, `SettingsDialog.test.ts` |
...
| SET-012 | Azure AI provider 页面展示 `Deployment name / API version` 专属字段，并支持保存 Azure 配置 | `SettingsDialog.execution.test.tsx` |
...
| SET-018 | Configure execution 页面里的 `AI assistant_CONFIG_DIR`、`code agent_HOME` 可保存进配置 | `SettingsDialog.execution.test.tsx`, `SettingsDialog.test.ts` |
...
| SET-038 | tool server 页面在 daemon 返回 install info 后，会默认渲染 code agent 的安装命令、重启提示和能力说明 | `SettingsDialog.execution.test.tsx` |
| SET-039 | tool server 页面切换不同 client 后，会联动更新安装方式说明和 snippet 内容 | `SettingsDialog.execution.test.tsx` |
| SET-040 | tool server 页面支持复制当前 snippet 到剪贴板，并展示 `Copied` 反馈 | `SettingsDialog.execution.test.tsx` |
| SET-041 | tool server 页面在 daemon 无法返回 install info 时，会展示错误提示和降级 snippet 文案 | `SettingsDialog.execution.test.tsx` |
...
| SET-C05 | tool server 的 code editor deeplink / 多平台路径差异（macOS/Linux/Windows） | 适合自动化，但需要更细的环境 mock 或浏览器 scheme 行为校验，适合后续补 |
...
| SET-C08 | Pets 页面上传 sprite、导入 code agent atlas、裁剪单行或保留 full atlas 的文件处理链路 | 适合自动化，但依赖文件输入、图片读取、canvas 裁剪和 atlas 预处理，维护成本更高 |
