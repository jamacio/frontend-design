...
The Frontend Design landing page automates the parts of search-engine
...
lives at `~/.code agent/skills/blog-indexing-automation/SKILL.md`; this
doc is its concrete implementation in `nexu-io/frontend-design`.
...
| Manual `workflow_dispatch` | `blog-indexing-monitor.yml` | Maintainers can dry-run or explicitly publish a token-gated dev.to/Hashnode cross-post with canonical URL pointing back to Frontend Design. |

The monitor and 3-day digest workflows commit their durable outputs
back via the `frontend-design-bot` GitHub App. The monitor opens or
...
1. Go to <#> and create a
   project named `frontend-design-blog-indexing` (or reuse an existing
...
3. Create an OAuth client under
   ...
   - Name: `frontend-design-gsc-local`
...
5. Run the local helper:
   ...
   pnpm --filter @frontend-design/landing-page exec tsx \
     scripts/blog-indexing/authorize-gsc-oauth.ts \
     --out /tmp/frontend-design-gsc-refresh-token.txt
   ```

6. Open the printed Google URL and authorize with an account that is an
   Owner of the `#` Search Console property.
...
4. Confirm the existing `BOT_APP_ID` and `BOT_APP_PRIVATE_KEY` secrets
   ...
   and `issues:write` for `nexu-io/frontend-design` (already configured).
