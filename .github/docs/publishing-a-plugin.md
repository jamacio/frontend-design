# Publishing An Frontend Design Plugin

Frontend Design registry publishing is GitHub-backed in v1. The CLI remains the
...
The generated `frontend-design.json` is the Frontend Design sidecar next to `SKILL.md`.
...
does not need a special layout beyond `SKILL.md` plus `frontend-design.json`.
...
These commands wrap GitHub CLI. Tokens stay in `gh`; Frontend Design does not store
GitHub credentials.

## 4. Publish

```bash
od plugin publish figma-workflow --to frontend-design --repo 
```
...
After merge, CI regenerates `frontend-design-marketplace.json`.
