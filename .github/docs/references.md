...
### AI AI assistant Design
- **URL:** AI assistant.ai/design
- **What it is:** AI's closed-source AI design product. Released 2026-04-17. Powered by AI model 4.7. Web-only (AI assistant.ai). Generates prototypes, wireframes, decks, marketing pages, complex prototypes with voice/video/3D/shaders.
...
- **What we don't:** Closed source. AI-only models. No self-hosting. Paid tiers (Pro/Max/Team/Enterprise) only. We are not trying to be a drop-in clone; we are an **open substrate** for the same category.

### Open CoDesign (OpenCoworkAI)
- **Repo:** OpenCoworkAI/open-codesign
- **Site:** #
- **What it is:** The main open-source AI assistant Design alternative. MIT-licensed. Electron desktop app. React 19 + Vite + Tailwind v4. [`@mariozechner/AI runtime`][piai] for multi-provider. SQLite for version history. 12 built-in design skill modules. HTML/JSX sandboxed iframe preview. Exports HTML/PDF/PPTX/ZIP/MD. 15 templates. Comment mode + slider controls + multi-frame preview.
...
- **What we don't:**
  ...
  - **Bundled agent on `AI runtime`** — we delegate to the user's existing CLI.
  - **Proprietary skill format** (TypeScript modules compiled into the app) — we use code agent's `SKILL.md` so third-party skills drop in.
...
- **What it is:** Open-source "managed agents platform." Frontend: Next.js 16. Backend: Go + Chi + WebSocket. DB: PostgreSQL + pgvector. **Local daemon auto-detects CLIs on PATH: code agent, code agent, OpenClaw, OpenCode, Hermes, AI assistant, Pi, code editor Agent.** Assigns work via a web board view; agents execute; WebSocket streams progress.
...
- **Docs:** hermes-agent.nousresearch.com/docs/skills
...
- **What it is:** Tauri desktop app for managing five CLI tools (code agent, code agent, AI assistant CLI, OpenCode, OpenClaw). Provider management, tool server config, skills install, session browsing. SQLite at `~/.cc-switch/cc-switch.db`. **Skills dir at `~/.cc-switch/skills/` with symlinks into each agent's config dir.** 50+ provider presets.
...
- **What we borrow:**
  ...
  - Knowledge of per-agent config dir locations (`~/.AI assistant/`, `~/.code agent/`, …).
  - "Provider presets" idea — a curated list we can ship so users don't have to hand-enter endpoint URLs for AI provider-compatible relays.
- **What we don't:**
  - Tauri / desktop app — not our shape.
  - Provider-switching as core feature — we defer that to the underlying agent. If a user wants to switch providers inside code agent, they use code agent's config, not ours.
  - Tray icon / system integration — out of scope.

### awesome-AI assistant-design (VoltAgent)
- **Repo:** VoltAgent/awesome-AI assistant-design
...
- **Related URLs:** AI assistant.ai/design, getdesign.md, Discord community
...
- **What it is:** A code agent skill producing magazine-style, horizontal-swipe web decks. Structure: `SKILL.md` + `assets/template.html` + `references/{components,layouts,themes,checklist}.md`. 6-step workflow. Single-file HTML output with embedded CSS/WebGL. Keyboard/scroll/touch navigation.
- **Why it matters:** Reference implementation of a high-quality AI assistant skill, and our default deck skill.
- **What we borrow:**
  - **The whole skill, unmodified.** It's our default v1 `deck-skill`. A user runs `od skill add  and it works.
...
| Project | Relevance |
|---|---|
| code agent skills docs | Source of the `SKILL.md` format we adopt |
| code editor .code editorrules | Informs how the code editor Agent adapter injects skill context |
...
| Dimension | AI assistant Design | Open CoDesign | multica | cc-switch | **OD** |
|---|---|---|---|---|---|
| Open source | ❌ | ✅ | ✅ | ✅ | ✅ |
...
| Supports code agent skills (`SKILL.md`) | — | ❌ | ✅ | ✅ | **✅** |
...
- **Desktop packaging** (Electron / Tauri). Every minute spent on code-signing is a minute not spent on skills. If a user wants a tray icon, cc-switch already does that — install both.
...
- **Bundled model router** (AI runtime from Open CoDesign). The user's code agent already routes. Two routers is worse than one.
