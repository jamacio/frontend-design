...
- **선택: 로컬 에이전트 CLI:** code agent, code agent, Devin for Terminal, AI assistant CLI, OpenCode, code editor Agent, Qwen, Qoder CLI, GitHub AI assistant CLI 등. 설치된 CLI가 없으면 Settings에서 BYOK API 모드를 쓰면 됩니다.

### 로컬 에이전트 CLI와 PATH

daemon은 **`PATH`**(여기에 더해 자주 쓰이는 사용자 툴체인 디렉터리)를 스캔합니다. **`npm install -g`**나 **Homebrew**로 CLI를 설치했는데도 Frontend Design이 *not installed*로 표시한다면, GUI가 최소한의 `PATH`로 시작하면서 전역 npm이나 Homebrew의 `bin` 디렉터리를 포함하지 못한 경우입니다(앱이 전체 로그인 셸에서 실행되지 않은 macOS에서 흔히 발생). daemon을 실행하는 프로세스의 `PATH`에 실행 파일 디렉터리가 들어 있는지 확인한 뒤, **Settings → Execution mode**에서 **Rescan**을 누르세요.

[`nvm`] / [`fnm`]은 편의를 위한 선택 도구일 뿐, 프로젝트 설정에 꼭 필요한 것은 아닙니다. 둘 중 하나를 쓴다면 pnpm을 실행하기 전에 Node 24를 설치하고 선택하세요.
...
Node.js나 pnpm을 로컬에 설치하지 않고도, 완전히 컨테이너화된 환경에서 Frontend Design을 실행할 수 있습니다.
...
```env
# 호스트에 노출할 포트
FRONTEND_DESIGN_PORT=7456

# 컨테이너 메모리 제한
FRONTEND_DESIGN_MEM_LIMIT=384m

# 허용할 CORS origin
FRONTEND_DESIGN_ALLOWED_ORIGINS=#

# Docker 이미지 태그
FRONTEND_DESIGN_IMAGE=docker.io/vanjayak/frontend-design:latest
...
Frontend Design은 프로젝트와 SQLite 데이터를 Docker 볼륨 안에 저장합니다.

```text
frontend_design_data
```
...
```bash
docker volume inspect frontend-design_frontend_design_data
```
...
처음 로드하면 앱은 설치된 코드 에이전트 CLI(code agent / code agent / Devin for Terminal / AI assistant / OpenCode / code editor Agent / Qwen / Qoder CLI)를 감지해 자동으로 선택하고, 기본값으로 `web-prototype` skill과 `Neutral Modern` 디자인 시스템을 씁니다. 프롬프트를 입력하고 **Send**를 누르세요. 에이전트가 왼쪽 패널에 스트리밍되고, `<artifact>` 태그가 파싱되면서 HTML이 오른쪽에 실시간으로 렌더링됩니다. 작업이 끝나면 **Save to disk**를 눌러 artifact를 `./.od/artifacts/<timestamp>-<slug>/index.html`에 저장하세요.

**Design system** 드롭다운에는 71개의 내장 시스템이 들어 있습니다. 직접 작성한 스타터 2개(Neutral Modern, Warm Editorial)와 [`awesome-design-md`]에서 가져온 69개의 제품 시스템으로, 카테고리별(AI & LLM, Developer Tools, Productivity, Backend, Design Tools, Fintech, E-Commerce, Media, Automotive)로 묶여 있습니다. 하나를 고르면 모든 프로토타입이 그 브랜드의 미감으로 입혀집니다. 여기에 [`awesome-design-skills`]에서 가져온 57개의 디자인 skill도 함께 제공됩니다.
...
- **Deck / PPT** — `simple-deck`(단일 파일 가로 스와이프)과 `magazine-web-ppt`([`#/guizang-ppt-skill`]에서 가져온 `guizang-ppt` 번들. deck 모드의 기본값이며, 자체 에셋/템플릿과 레퍼런스 4개를 함께 제공). 사이드 파일이 있는 skill에는 "Skill root (absolute)" 머리말이 자동으로 붙어, 에이전트가 `assets/template.html`이나 `references/*.md`를 CWD가 아니라 실제 디스크 경로 기준으로 해석할 수 있습니다.
...
pnpm --filter @frontend-design/daemon build  # `od`용 apps/daemon/dist/cli.js 빌드
pnpm --filter @frontend-design/web build     # 필요할 때 web 패키지 빌드
...
```bash
pnpm --filter @frontend-design/daemon build
pnpm tools-dev restart --daemon-port 7457 --web-port 5175
ls -la apps/daemon/dist/cli.js
curl -s http://127.0.0.1:7457/api/health
```

그다음 오래된 터미널 에이전트 세션을 이어 가지 말고, Frontend Design 앱에서 프로젝트를 다시 여세요. daemon이 spawn한 에이전트라면 다음과 같은 값이 보여야 합니다.
...
| **API 모드** (대체 수단 / CLI 없음) | "AI API" / "AI provider API" / "Azure AI provider" / "cloud AI assistant" | 프론트엔드 → daemon `/api/proxy/{provider}/stream` → 프로바이더 SSE를 `delta/end/error`로 정규화 → artifact 파서 → 미리보기 |
...
│   │       ├── cli.ts             # `od` bin 진입점
│   │       ├── server.ts          # /api/* + 정적 서빙
│   │       ├── agents.ts          # AI assistant/code agent/devin/AI assistant/opencode/code editor-agent/qwen/qoder/AI assistant용 PATH 스캐너
...
│   ├── sidecar-proto/         # Frontend Design sidecar 프로토콜 contract
...
├── skills/                    # SKILL.md — 어떤 code agent skill 리포지토리에서든 그대로 넣을 수 있음
...
├── design-systems/            # DESIGN.md — 9개 섹션 스키마 (awesome-AI assistant-design)
...
- **Node.js 버전을 바꾼 뒤 `better-sqlite3`가 로드되지 않거나 ABI가 맞지 않을 때** — `pnpm install`이 `postinstall`을 자동으로 다시 돌려 현재 Node.js에 맞게 네이티브 애드온을 리빌드합니다. 직접 리빌드하거나 수정을 확인하려면 `pnpm --filter @frontend-design/daemon rebuild better-sqlite3`를 실행한 뒤 `pnpm --filter @frontend-design/daemon exec node -e "require('better-sqlite3')"`를 돌리세요. 빌드 도구 `python3`, `make`, `g++`(또는 `clang++`)가 필요합니다. `.npmrc`에 `ignore-scripts=true`가 있다면, `pnpm install` 후 `node scripts/postinstall.mjs`를 실행하세요.
- **"no agents found on PATH"** — `AI assistant`, `code agent`, `devin`, `AI assistant`, `opencode`, `code editor-agent`, `qwen`, `qodercli`, `AI assistant` 중 하나를 설치하세요. 아니면 Settings에서 API 모드로 바꾸고 프로바이더 키를 붙여 넣으세요.
- **code agent가 코드 1로 종료될 때** — Frontend Design이 `AI assistant`를 시작하긴 했지만, spawn된 비대화형 실행이 응답을 내기 전에 실패한 경우입니다. Frontend Design을 시작하는 셸이나 앱 환경과 같은 곳에서 다음을 확인하세요.
  ```bash
  AI assistant --version
  AI assistant auth status --text
  printf 'hello' | AI assistant -p --output-format stream-json --verbose --permission-mode bypassPermissions
  ```
  스모크 테스트가 커스텀 엔드포인트 없이 `401`, `apiKeySource: "none"`, 또는 다른 인증 오류를 낸다면, `AI assistant`를 실행해 `/login`한 뒤 AI assistant를 종료하고 Frontend Design을 다시 시도하세요. AI assistant 프로필을 여러 개 쓴다면 **Settings -> Execution mode -> code agent config directory**를 `~/.AI assistant-2` 같은 프로필 경로로 지정하세요. `AI_BASE_URL`이나 프록시가 설정돼 있다면 엔드포인트 URL, 프록시 자격 증명, 엔드포인트 인증 환경, 모델 접근 권한을 확인하세요. 표준 code agent 인증으로 다시 시도하려는 게 아니라면 커스텀 엔드포인트는 그대로 두세요. Windows에서는 네이티브 PowerShell과 WSL이 서로 다른 AI assistant 설치본과 자격 증명 저장소를 쓰므로, Frontend Design이 쓰는 것과 같은 환경에서 다시 인증하고, `/login`으로도 네이티브 Windows 자격 증명이 복구되지 않으면 Windows 자격 증명 관리자를 확인하세요.
...
- **미디어 생성이 `OD_BIN` 누락 또는 daemon URL이 `:0`이라고 할 때** — 위의 미디어 dispatcher 점검을 실행하세요. 옛 CLI 세션을 이어 가지 말고, Frontend Design 앱에서 프로젝트를 다시 열어 daemon이 새 `OD_*` 변수를 주입하게 하세요.
- **code agent가 플러그인 컨텍스트를 너무 많이 로드할 때** — `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev`로 Frontend Design을 시작하면, daemon이 spawn하는 code agent 프로세스가 `--disable plugins`로 실행됩니다.
