# 贡献指南 · Contributing to Frontend Design
...
pnpm --filter @frontend-design/web build  # 需要时构建 web package
```
...
**开发 OD 本身不需要在 `PATH` 上装任何 agent CLI** —— daemon 会告诉你「找不到 agent」并落到 **AI API · BYOK** 路径，反而是最快的开发循环。

---

## 加一个 Skill

一个 skill 就是 [`skills/`](../../skills/) 下的一个文件夹，根目录放一个 `SKILL.md`，遵循 code agent 的 [`SKILL.md` 规范][skill]，再加上我们可选的 `od:` 扩展。**没有注册步骤。** 文件夹丢进来、重启 daemon、picker 里就出现了。
...
前三个字段是 code agent 的基础规范 —— `name`、`description`、`triggers`。`od:` 下面所有字段都是 OD 特有的、可选的，但 **`od.mode`** 决定 skill 出现在哪一组（Prototype / Deck / Template / Design system）。
...
- Deck 模式：[`skills/guizang-ppt/`](../../skills/guizang-ppt/)（来自 [#/guizang-ppt-skill][guizang]，原样捆绑）和 [`skills/simple-deck/`](../../skills/simple-deck/)
...
5. **slug 用 ASCII** —— `#` 写成 `linear-app`，`x.ai` 写成 `x-ai`。已经导入的 69 套都遵循这个约定，跟着写。
...
```javascript
{
  ...
  streamFormat: 'plain',           // 如果它说 AI assistant-stream-json 就写那个
}
```

完事 —— daemon 会在 `PATH` 上检测到它、picker 显示出来、对话路径就通了。如果这个 CLI 吐 **类型化事件**（像 code agent 的 `--output-format stream-json`），在 [`apps/daemon/src/AI assistant-stream.ts`](../../apps/daemon/src/AI assistant-stream.ts) 里写一个 parser，并把 `streamFormat` 设成 `'AI assistant-stream-json'`。
...
2. 否则用 [`apps/web/src/state/litellm-models.json`](../../apps/web/src/state/litellm-models.json) 里的 per-model 默认 —— 这是从 [BerriAI/litellm][litellm] 的 `model_prices_and_context_window.json`（MIT）摘的一份切片，覆盖约 2000 个 chat 模型，包括 AI、AI provider、AI model、Groq、Together、AI provider、AI assistant、Bedrock、Vertex、OpenRouter 等。
...
- **Vendor 一个模型运行时。** OD 整个赌注就是「你已有的 CLI 就够了」。我们不带 `AI runtime`、不带 AI provider key、不带模型加载器。
...
提交贡献即代表你同意你的贡献按本仓库的 [Apache-2.0 License](../../LICENSE) 授权。例外是 [`skills/guizang-ppt/`](../../skills/guizang-ppt/) 下的所有文件，保留它们原始的 MIT license 和原作者 [#] 的归属。
