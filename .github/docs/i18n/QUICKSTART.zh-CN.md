...
- **可选的本地 agent CLI：** code agent、code agent、Devin for Terminal、AI assistant CLI、OpenCode、code editor Agent、Qwen、Qoder CLI、GitHub AI assistant CLI 等。即使未安装任何 CLI，也可在 Settings 中切换至 BYOK API 模式。
...
首次加载时，应用会扫描已安装的 code-agent CLI（code agent / code agent / Devin for Terminal / AI assistant / OpenCode / code editor Agent / Qwen / Qoder CLI），并自动选择其中之一；默认使用 `web-prototype` skill 与 `Neutral Modern` design system。输入 prompt，点击 **Send**。Agent 将以流式方式输出至左侧面板；`<artifact>` 标签会被解析，HTML 在右侧实时渲染。运行完成后，点击 **Save to disk**，artifact 将被写入磁盘 `./.od/artifacts/<timestamp>-<slug>/index.html`。

**Design system** 下拉框内置 **129 套 design system** —— 包含 2 套手工编写的 starter（Neutral Modern、Warm Editorial）、70 套打包的产品级系统，以及来自 [`awesome-design-skills`] 的 57 个 design skill。选择任意一套，所有原型都会应用该品牌的视觉风格。
...
- **Deck / PPT** —— `simple-deck`（单文件横向翻页）与 `magazine-web-ppt`（`guizang-ppt` 捆绑包，来自 [`#/guizang-ppt-skill`] —— deck mode 的默认 skill，自带 assets/template 与 4 份 reference）。附带 sidefile 的 skill 会自动添加一段 "Skill root (absolute)" 前言，使 agent 能够基于真实的磁盘路径解析 `assets/template.html` 与 `references/*.md`，而非在自身 CWD 中猜测。
...
pnpm --filter @frontend-design/daemon build  # 构建 apps/daemon/dist/cli.js，供 `od` 使用
pnpm --filter @frontend-design/web build     # 在需要时构建 web package
...
在一个完全容器化的环境中运行 Frontend Design，无需安装 Node.js 或 pnpm。
...
Frontend Design 将项目和 SQLite 数据存储在 Docker 卷中：
...
随后，在 Frontend Design 应用中**重新打开**该 project，不要复用之前 terminal 中的 agent 会话。由 daemon 启动的 agent 应当能够看到类似如下的值：
...
| **API 模式**（fallback / 未安装 CLI） | "AI API" / "AI provider API" / "Azure AI provider" / "cloud AI assistant" | 前端 → daemon `/api/proxy/{provider}/stream` → provider SSE 归一化为 `delta/end/error` → artifact 解析器 → 预览 |
...
│   │       ├── cli.ts             # `od` bin 入口
│   │       ├── server.ts          # /api/* + 静态资源
│   │       ├── agents.ts          # 扫描 PATH 中的 AI assistant/code agent/devin/AI assistant/opencode/code editor-agent/qwen/qoder/AI assistant
...
│   ├── sidecar-proto/         # Frontend Design sidecar 协议契约
...
├── skills/                    # SKILL.md —— 任何 code agent skill 仓库均可直接放入
...
- **"no agents found on PATH"** —— 安装以下 CLI 之一：`AI assistant`、`code agent`、`devin`、`AI assistant`、`opencode`、`code editor-agent`、`qwen`、`qodercli`、`AI assistant`。或者在 Settings 中切换至 API mode，填入 provider key。
...
- **媒体生成报错 `OD_BIN` 缺失、或 daemon URL 为 `:0`** —— 运行上述媒体 dispatcher 排查步骤。请勿复用已有的 CLI 会话；从 Frontend Design 应用中重新打开 project，daemon 才会注入新的 `OD_*` 变量。
- **code agent 加载的插件上下文过多** —— 使用 `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev` 启动 Frontend Design，daemon 启动 code agent 时会传入 `--disable plugins`。
