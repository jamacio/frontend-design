...
- **CLI d’agent locale optionnelle :** code agent, code agent, Devin for Terminal, AI assistant CLI, OpenCode, code editor Agent, Qwen, GitHub AI assistant CLI, etc. Si aucune n’est installée, utilisez le mode BYOK API depuis Settings.
...
pnpm --filter @frontend-design/daemon build  # build apps/daemon/dist/cli.js pour `od`
pnpm --filter @frontend-design/web build     # build du paquet web si nécessaire
...
Exécutez Frontend Design dans un environnement entièrement conteneurisé sans installer Node.js ou pnpm localement.
...
Frontend Design stocke les projets et les données SQLite dans un volume Docker :
...
Ouvrez ensuite de nouveau le projet depuis l’app Frontend Design au lieu de reprendre une ancienne session agent dans le terminal. Un agent lancé par le daemon devrait voir des valeurs comme :
...
| **Mode API** (fallback / aucune CLI) | "AI API" / "AI provider API" / "Azure AI provider" / "cloud AI assistant" | Frontend → daemon `/api/proxy/{provider}/stream` → SSE provider normalisé en `delta/end/error` → parser `<artifact>` → preview |
...
│   ├── sidecar-proto/         # contrat du protocole sidecar Frontend Design
...
├── skills/                    # SKILL.md — drop-in depuis n’importe quel repo code agent skill
...
├── design-systems/            # DESIGN.md — schéma 9 sections (awesome-AI assistant-design)
...
- **"no agents found on PATH"** — installez une CLI compatible, par exemple `AI assistant`, `code agent`, `AI assistant`, `opencode`, `code editor-agent`, `qwen` ou `AI assistant`. La liste exacte des adapters détectés vit dans `apps/daemon/src/agents.ts`. Ou passez au mode API/BYOK dans la barre supérieure et collez une clé dans **Settings**.
...
- **la génération média dit que `OD_BIN` manque ou que l’URL daemon vaut `:0`** — exécutez les checks du dispatcher média ci-dessus. Ne reprenez pas l’ancienne session CLI ; rouvrez le projet depuis l’app Frontend Design pour que le daemon injecte des variables `OD_*` fraîches.
- **code agent charge trop de contexte plugin** — démarrez Frontend Design avec `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev` pour que les processus code agent lancés par le daemon tournent avec `--disable plugins`.
