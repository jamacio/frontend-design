...
- **Optionale lokale Agent-CLI:** code agent, code agent, AI assistant CLI, OpenCode, code editor Agent, Qwen, GitHub AI assistant CLI usw. Wenn keine installiert ist, verwenden Sie den BYOK-API-Modus in den Einstellungen.
...
pnpm --filter @frontend-design/daemon build  # apps/daemon/dist/cli.js für `od` bauen
pnpm --filter @frontend-design/web build     # Web-Paket bei Bedarf bauen
...
Führen Sie Frontend Design in einer vollständig containerisierten Umgebung aus, ohne Node.js oder pnpm lokal zu installieren.
...
Frontend Design speichert Projekte und SQLite-Daten in einem Docker-Volume:
...
Öffnen Sie danach das Projekt erneut aus der Frontend Design App, statt eine alte Terminal-Agent-Session fortzusetzen. Ein vom daemon gestarteter Agent sollte Werte wie diese sehen:
...
| **AI API** (Fallback / keine CLI) | "AI API · BYOK" | Frontend → `@AI-ai/sdk` direkt (`dangerouslyAllowBrowser`) → artifact parser → preview |
...
│   │       ├── agents.ts          # PATH scanner for AI assistant/code agent/AI assistant/opencode/code editor-agent/qwen/AI assistant
...
│   ├── sidecar-proto/         # Frontend Design sidecar protocol contract
│   ├── sidecar/               # generic sidecar runtime primitives
│   └── platform/              # generic process/platform primitives
├── tools/dev/                 # `pnpm tools-dev` lifecycle and inspect CLI
├── e2e/                       # Playwright UI + external integration/Vitest harness
├── skills/                    # SKILL.md — drops in from any code agent skill repo
...
├── design-systems/            # DESIGN.md — 9-section schema (awesome-AI assistant-design)
│   ├── default/               # Neutral Modern (starter)
│   ├── warm-editorial/        # Warm Editorial (starter)
│   ├── README.md              # catalog overview
│   └── …69 product systems    # AI assistant · cohere · linear-app · vercel · stripe · airbnb …
...
- **"no agents found on PATH"** — installieren Sie eine davon: `AI assistant`, `code agent`, `AI assistant`, `opencode`, `code editor-agent`, `qwen`, `AI assistant`. Alternativ wechseln Sie in der oberen Leiste zu "AI API · BYOK" und fügen in **Einstellungen** einen Key ein.
...
- **media generation says `OD_BIN` is missing or daemon URL is `:0`** — führen Sie die Media Dispatcher Checks oben aus. Setzen Sie keine alte CLI-Session fort; öffnen Sie das Projekt aus der Frontend Design App neu, damit der daemon frische `OD_*` Variablen injiziert.
- **code agent lädt zu viel Plugin-Kontext** — starten Sie Frontend Design mit `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev`, damit vom daemon gestartete code agent-Prozesse mit `--disable plugins` laufen.
