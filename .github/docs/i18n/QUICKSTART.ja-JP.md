...
- **オプションのローカルエージェント CLI:** code agent、code agent、Devin for Terminal、AI assistant CLI、OpenCode、code editor Agent、Qwen、GitHub AI assistant CLI など。何もインストールされていない場合は、設定から BYOK API モードを使用してください。
...
初回起動時、アプリはインストール済みのコードエージェント CLI（code agent / code agent / Devin for Terminal / AI assistant / OpenCode / code editor Agent / Qwen）を検出して自動選択し、デフォルトで `web-prototype` スキルと `Neutral Modern` デザインシステムを採用します。プロンプトを入力して **Send** を押してください。エージェントが左ペインにストリーミングし、`<artifact>` タグが解析されて HTML が右側にライブレンダリングされます。完了したら **Save to disk** をクリックして、アーティファクトを `./.od/artifacts/<timestamp>-<slug>/index.html` に永続化します。

**Design system** ドロップダウンには **129 のデザインシステム** が同梱されています — 手作りのスターター 2 種（Neutral Modern、Warm Editorial）、バンドルされた製品システム 70 種、[`awesome-design-skills`] から取得した 57 のデザインスキルです。1 つを選ぶと、すべてのプロトタイプがそのブランドの美学でスキニングされます。
...
- **Deck / PPT** — `simple-deck`（単一ファイルの横スワイプ）と `magazine-web-ppt`（[`#/guizang-ppt-skill`] からの `guizang-ppt` バンドル — deck モードのデフォルト。独自のアセット／テンプレート + 4 つのリファレンスを同梱）。サイドファイルを持つスキルには自動的に「Skill root (absolute)」のプリアンブルが付与され、エージェントが CWD ではなく実際のディスク上のパスに対して `assets/template.html` や `references/*.md` を解決できるようになります。
...
pnpm --filter @frontend-design/daemon build  # `od` 用に apps/daemon/dist/cli.js をビルド
pnpm --filter @frontend-design/web build     # 必要に応じて web パッケージをビルド
...
Node.js や pnpm をローカルにインストールせずに、完全にコンテナ化された環境で Frontend Design を実行できます。
...
Frontend Design はプロジェクトと SQLite データを Docker ボリュームに保存します：
...
その後、古いターミナルエージェントセッションを再開する代わりに、Frontend Design アプリからプロジェクトを再度開いてください。daemon から起動されたエージェントは、次のような値を確認できるはずです：
...
| **AI API**（フォールバック / CLI なし） | "AI API · BYOK" | フロントエンド → `@AI-ai/sdk` 直接呼び出し（`dangerouslyAllowBrowser`） → アーティファクトパーサー → プレビュー |
...
│   │       ├── cli.ts             # `od` bin エントリ
│   │       ├── server.ts          # /api/* + 静的配信
│   │       ├── agents.ts          # AI assistant/code agent/devin/AI assistant/opencode/code editor-agent/qwen/AI assistant 用 PATH スキャナ
...
│   ├── sidecar-proto/         # Frontend Design sidecar プロトコル契約
...
├── skills/                    # SKILL.md — 任意の code agent スキルリポジトリからドロップイン
...
├── design-systems/            # DESIGN.md — 9 セクションスキーマ（awesome-AI assistant-design）
...
- **「no agents found on PATH」** — `AI assistant`、`code agent`、`devin`、`AI assistant`、`opencode`、`code editor-agent`、`qwen`、`AI assistant` のいずれかをインストールしてください。または、トップバーで「AI API · BYOK」に切り替え、**設定** にキーを貼り付けます。
...
- **メディア生成で `OD_BIN` が欠落、または daemon URL が `:0`** — 上記のメディアディスパッチャーチェックを実行してください。古い CLI セッションを再開せず、Frontend Design アプリからプロジェクトを再度開いて、daemon が新しい `OD_*` 変数を注入できるようにしてください。
- **code agent がプラグインコンテキストを多く読み込みすぎる** — `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev` で Frontend Design を起動すると、daemon から起動された code agent プロセスが `--disable plugins` で実行されます。
