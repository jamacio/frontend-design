...
Wenn Sie nicht sicher sind, in welchen Bereich Ihre Idee fällt, [öffnen Sie zuerst eine Discussion / Issue]. Wir zeigen Ihnen dann die passende Oberfläche.
...
Sie brauchen keine Agent-CLI im `PATH`, um OD selbst zu entwickeln. Der daemon meldet dann "no agents found" und fällt auf den **AI API · BYOK** Pfad zurück, der oft die schnellste Dev-Schleife ist.

---

## Einen neuen Skill hinzufügen

Ein Skill ist ein Ordner unter [`skills/`](../../skills/) mit `SKILL.md` im Root. Er folgt der code agent [`SKILL.md` Konvention][skill] plus optionaler `od:` Erweiterung. **Keine Registrierung nötig.** Ordner ablegen, daemon neu starten, der Picker zeigt ihn an.
...
Die ersten drei Keys sind die code agent Basis-Spec: `name`, `description`, `triggers`. Alles unter `od:` ist OD-spezifisch und optional, aber **`od.mode`** bestimmt, in welcher Gruppe der Skill erscheint.
...
5. **Slug nutzt ASCII** — `#` wird `linear-app`, `x.ai` wird `x-ai`.
...
Der daemon erkennt sie im `PATH`, der Picker zeigt sie an und der Chat-Pfad funktioniert. Wenn die CLI **typed events** ausgibt, ergänzen Sie einen Parser in [`apps/daemon/src/AI assistant-stream.ts`](../../apps/daemon/src/AI assistant-stream.ts) und setzen `streamFormat`.
...
- Architekturfrage, Designfrage, "Bug oder Fehlbenutzung?" → [GitHub Discussions] (bevorzugt, weil suchbar).
...
Mit Ihrem Beitrag erklären Sie sich einverstanden, dass er unter der [Apache-2.0-Lizenz](../../LICENSE) dieses Repositories steht. Ausnahme sind Dateien in [`skills/guizang-ppt/`](../../skills/guizang-ppt/), die ihre ursprüngliche MIT-Lizenz und Autorenschaft von [#] behalten.
