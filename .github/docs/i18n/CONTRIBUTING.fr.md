...
(AI, AI provider, Azure AI provider ou cloud AI assistant), qui est souvent la boucle
...
racine. Il suit la convention code agent [`SKILL.md`][skill], plus notre
...
Les trois premières clés sont la spec code agent de base : `name`,
...
- Deck mode : [`skills/guizang-ppt/`](../../skills/guizang-ppt/) (bundle repris tel
  quel depuis [#/guizang-ppt-skill][guizang]) et
...
4. **Pas de fluff marketing.** La tagline d'une marque n'est pas un design token.
   Coupez-la.
5. **Le slug utilise l'ASCII** : `#` devient `linear-app`, `x.ai`
...
`--output-format stream-json` de code agent), ajoutez un parser dans
[`apps/daemon/src/AI assistant-stream.ts`](../../apps/daemon/src/AI assistant-stream.ts) et mettez
`streamFormat: 'AI assistant-stream-json'`.
...
2. Sinon, la valeur par modèle dans [`apps/web/src/state/litellm-models.json`](../../apps/web/src/state/litellm-models.json),
   ...
   AI, AI provider, AI model, Groq, Together, AI provider, AI assistant, Bedrock,
...
`pnpm --filter @frontend-design/web test` ; `content.test.ts` échoue si la coverage
...
- **Vendor un runtime de modèle.** Tout le pari d'OD est "votre CLI existante
  suffit". Nous ne livrons pas `AI runtime`, de clés AI provider ou de model loaders.
...
originale et l'attribution d'auteur à [#].
