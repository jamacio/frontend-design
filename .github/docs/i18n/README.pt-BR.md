<h1 align="center">Frontend Design: a alternativa open-source ao AI assistant Design</h1>

> 🔥 **O Frontend Design 0.10.0 chegou: o espaço de trabalho de design Agentic tudo-em-um.** Todo o ofício agora vive em uma única janela — vá de uma ideia vaga à descoberta de referências, coleta de material, edição interativa, fila de comentários, polimento de animações e entrega para um editor ou um Code Agent — sem sair do app. Com sessões em paralelo, deixa de parecer um assistente e passa a parecer um time de design local trabalhando para você. [Baixar 0.10.0] · [Participar da discussão]
>
> ⚡ **Frontend Design AMR (Agentic Model Router) — o serviço de modelos oficial.** Uma única recarga para usar AI, AI assistant, AI assistant e AI model dentro do Frontend Design: mais de 20 modelos de ponta, zero configuração, cobrança pelo uso real de tokens. [Experimentar o AMR](#)
>
> 🏅 **O programa Frontend Design Fellow está aberto.** Se você também acredita que o design deveria ser aberto — torne-se um Frontend Design Fellow, ajude a moldar o produto ao lado do time central e contribua para que mais pessoas participem da definição do futuro do design. Detalhes → [`MAINTAINERS.md`](../../MAINTAINERS.md) e [Discord](#).
...
## O que é o Frontend Design

🎨 **A alternativa local-first e open-source ao [AI assistant Design][cd].** &nbsp;🖥️ **App de desktop nativo para macOS e Windows.** &nbsp;⚡ **Mais de 100 skills** · ✨ **150 sistemas `DESIGN.md` de nível profissional de marca** · 📦 **261 plugins prontos para uso.** &nbsp;🖼️ Gera **protótipos web · desktop · mobile**, **dashboards / artefatos ao vivo**, **decks**, **imagens**, **vídeo**, além de motion graphics com **HyperFrames**. 🔒 Preview em iframe em sandbox · exportação para HTML / PDF / PPTX / MP4. &nbsp;🤖 **Roda em code agent · OpenClaw · code agent · code editor · OpenCode · Qwen · AI assistant · Hermes · Kimi · Antigravity e mais 21 CLIs locais**, ou em qualquer endpoint compatível com AI provider via BYOK.

O Frontend Design é o que você obtém quando o loop **agent-native** que a AI lançou com o AI assistant Design — descobrir o briefing, travar a direção, transmitir o artefato, criticar, entregar — deixa de ser fechado e se torna um **sistema de arquivos de skills, design systems e plugins** que os coding agents já presentes no seu laptop conseguem ler, escrever e remixar. Sua CLI vira o motor de design, seu laptop vira o estúdio e o `DESIGN.md` do seu time vira o contrato de marca.
...
<td width="50%" valign="top">
<sub><b>Integrations</b> — conecte sistemas externos e ferramentas MCP, e use o Frontend Design a partir de qualquer IDE, script ou automação.</sub>
</td>
</tr>
</table>
...
> O Frontend Design é entregue como **skills, uma CLI e um servidor MCP** que os coding agents populares consomem nativamente. Depois que o OD está instalado, um único `od mcp install <agent>` conecta o servidor MCP à configuração daquele agente, e você chama as mesmas ferramentas de dentro de qualquer agente.

| Coding agent / plataforma &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | Status &nbsp;&nbsp; | Instalação do servidor MCP em uma linha &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|---|:---:|---|
| [code agent](design documentation assistant-code) | ✅ Suportado | `od mcp install AI assistant` |
| [code agent CLI] | ✅ Suportado | `od mcp install code agent` |
| [code editor](# editor.com/cli) | ✅ Suportado | `od mcp install code editor` |
| [VS Code + GitHub AI assistant] | ✅ Suportado | `od mcp install AI assistant` |
| [GitHub AI assistant CLI] | ✅ Suportado | `od mcp install AI assistant` |
| [AI assistant CLI] | ✅ Suportado | `od mcp install AI assistant` |
| [OpenCode](#) | ✅ Suportado | `od mcp install opencode` |
| [OpenClaw] | ✅ Suportado | `od mcp install openclaw` |
| [Antigravity](https://#) | ✅ Suportado | `od mcp install antigravity` |
| [Cline] | ✅ Suportado | `od mcp install cline` |
| [Trae](https://#/) | ✅ Suportado | `od mcp install trae` |
| [Kimi CLI] | ✅ Suportado | `od mcp install kimi` |
| [Pi Agent] | ✅ Suportado | `od mcp install pi` |
| [AI provider Vibe CLI] | ✅ Suportado | `od mcp install vibe` |
...
**Nenhuma CLI instalada?** O proxy BYOK em `POST /api/proxy/{AI,AI provider,azure,google,oAI model,senseaudio}/stream` te dá o mesmo loop (sem spawn de processo) — cole `baseUrl` + `apiKey` + `model`, com suporte a AI provider, AI, Azure AI provider, cloud AI assistant, OAI model, LM Studio, vLLM ou qualquer endpoint compatível com AI provider. A proteção SSRF por alvo bloqueia IPs internos / link-local / CGNAT na borda do daemon.
...
<sub><b>Protótipo de app mobile</b> — um fluxo gamificado de três telas com faixas de XP e detalhe de quest. Faça o handoff direto para code editor / code agent / code agent para transformar em React/Next/Vue.</sub>
</td>
</tr>
</table>
...
<sub><b>Modo deck (guizang-ppt)</b> — layouts de revista, hero em WebGL, checklists P0/P1/P2. Empacotado na íntegra a partir de <a href=" com sua licença original preservada.</sub>
</td>
<td width="50%" valign="top">
...
### 4 · Imagens — `AI-image-2`, ImageRouter, API personalizada

<table>
<tr>
<td width="20%" valign="top"><img src="#" alt="Illustrated city food map" /><br/><sub><b>Mapa gastronômico ilustrado da cidade</b><br/>Pôster de viagem editorial desenhado à mão</sub></td>
<td width="20%" valign="top"><img src="#" alt="Cinematic elevator scene" /><br/><sub><b>Cena cinematográfica de elevador</b><br/>Still editorial de quadro único</sub></td>
<td width="20%" valign="top"><img src="#" alt="Cyberpunk anime portrait" /><br/><sub><b>Retrato cyberpunk</b><br/>Avatar de perfil — texto de rosto em neon</sub></td>
<td width="20%" valign="top"><img src="#" alt="3D stone staircase evolution" /><br/><sub><b>Escadaria de pedra 3D</b><br/>Infográfico em pedra talhada</sub></td>
<td width="20%" valign="top"><img src="#" alt="Glamorous portrait" /><br/><sub><b>Retrato glamoroso</b><br/>Foto de estúdio editorial</sub></td>
</tr>
</table>
...
O **[HyperFrames][hyperframes]** é o framework de vídeo open-source e agent-native da HeyGen, integrado como cidadão de primeira classe no Frontend Design. O agente escreve HTML + CSS + GSAP, e o HyperFrames renderiza tudo para um MP4 determinístico via Chrome headless + FFmpeg. Combine-o com o **Seedance 2.0** para t2v / i2v cinematográfico, **Veo 3 / Sora 2 / Kling 2** para variantes de modelo roteadas e **Suno v5 / Lyria 2** para a camada de áudio.

<table>
<tr>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-saas-product-promo-30s.json"><img src="#" alt="SaaS promo" /></a><br/><sub><b>Promo de produto SaaS de 30s</b> · 16:9 · reveals 3D de UI</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-tiktok-karaoke-talking-head.json"><img src="#" alt="TikTok karaoke" /></a><br/><sub><b>Talking-head de karaokê para TikTok</b> · 9:16 · TTS + legendas sincronizadas por palavra</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-brand-sizzle-reel.json"><img src="#" alt="Brand sizzle reel" /></a><br/><sub><b>Sizzle reel de marca de 30s</b> · 16:9 · tipografia cinética reativa ao áudio</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-data-bar-chart-race.json"><img src="#" alt="Bar chart race" /></a><br/><sub><b>Corrida de gráfico de barras</b> · 16:9 · infográfico de dados no estilo NYT</sub></td>
</tr>
<tr>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-flight-map-route.json"><img src="#" alt="Flight map" /></a><br/><sub><b>Mapa de voo</b> · 16:9 · reveal de rota no estilo Apple</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-logo-outro-cinematic.json"><img src="#" alt="Logo outro" /></a><br/><sub><b>Outro cinematográfico de logo de 4s</b> · 16:9 · montagem peça por peça + bloom</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-money-counter-hype.json"><img src="#" alt="Money counter" /></a><br/><sub><b>Contador de dinheiro $0 → $10K</b> · 9:16 · hype no estilo Apple</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-website-to-video-promo.json"><img src="#" alt="Website to video" /></a><br/><sub><b>Site para vídeo</b> · 16:9 · captura o site em 3 viewports</sub></td>
</tr>
</table>
...
## Por que o Frontend Design

> **Em abril de 2026, a AI lançou o [AI assistant Design][cd] — a primeira vez que um LLM deixou de escrever prosa e passou a entregar artefatos de design diretamente.** Viralizou. Mas continuou closed-source, pago, só na nuvem, preso ao modelo da AI, às skills da AI, à superfície da AI. Sem checkout, sem self-host, sem deploy na Vercel, sem trocar pelo seu próprio agente.

O Frontend Design (OD) é a alternativa open-source. Mesmo loop, mesmo modelo mental artifact-first, nada do lock-in:

- 🤖 **Agent-native, agnóstico de modelo.** Não entregamos um agente. O `AI assistant` / `code agent` / `code editor-agent` / `AI assistant` / `hermes` / `kimi` que já está no seu `PATH` é o motor de design. Troque com um clique.
- 🧠 **Nível profissional de marca por padrão.** Toda renderização lê o `DESIGN.md` ativo — um schema de 9 seções cobrindo paleta, tipografia, espaçamento, motion, voz e anti-padrões. 150 sistemas vêm com o repositório (Linear, Stripe, Vercel, Airbnb, Apple, Tesla, Notion, AI, code editor, Supabase, Figma…). Solte uma pasta e o seletor a encontra.
...
| Agent-native (roda na sua CLI) | Só AI | ❌ | Só agente na nuvem | **✅ 21 CLIs + BYOK** |
...
A forma mais rápida de usar o Frontend Design. Sem Node, sem pnpm, sem clone.

- **macOS** (Apple Silicon · Intel x64) → [**#**](#) ou [GitHub Releases]
- **Windows** (x64) → [**#**](#) ou [GitHub Releases]
...
Você pode usar o Frontend Design sem nunca abrir a GUI — chame-o como skill, plugin ou servidor MCP dentro do code agent, code agent, code editor, AI assistant, OpenClaw, Antigravity, Hermes, Kimi e mais.
...
4. **Faça o handoff para a engenharia.** O artefato é HTML/CSS real — solte-o no code editor, code agent ou code agent para continuar construindo como código. Ou exporte PPTX / PDF / MP4 direto para o marketing.
5. **O Frontend Design fica mais inteligente conforme você usa.** Seus screenshots, fontes, paletas e artefatos confirmados se acumulam como padrões para a próxima sessão. Menos retrabalho, menos desvio.

---

## Use o Frontend Design a partir do seu coding agent

O Frontend Design entrega um **servidor MCP stdio** e **scripts de instalação** por agente. Qualquer agente compatível com MCP em outro repositório pode ler arquivos diretamente dos seus projetos locais do Frontend Design — tokens CSS, componentes JSX, HTML de entrada — como uma API estruturada consultável por nome. O agente sempre vê o arquivo ao vivo, não uma exportação desatualizada.
...
**Para um agente começando do zero,** o instalador coloca `~/.config/<agent>/frontend-design.json` (ou o equivalente da plataforma) além de um snippet MCP de copiar e colar. O code editor recebe um deeplink de um clique; o code agent recebe um one-liner `AI assistant mcp add-json`; todo outro agente recebe JSON no schema que sua configuração espera. Fluxo completo por agente → **Settings → tool server** no app de desktop, ou [`docs/agent-adapters.md`](../../docs/agent-adapters.md).
...
**Mais de 100 skills vêm na caixa** — cada uma é uma pasta em [`skills/`](../../skills/) seguindo a convenção [`SKILL.md`][skill] do code agent, estendida com um frontmatter `od:` (`mode`, `platform`, `scenario`, `preview.type`, `design_system.requires`, `default_for`, `fidelity`, `example_prompt`). Solte uma pasta, reinicie o daemon e ela aparece no seletor.
...
**Developer Tools** — `code editor` · `vercel` · `linear-app` · `framer` · `expo` · `clickhouse` · `mongodb` · `supabase` · `hashicorp` · `posthog` · `sentry` · `warp` · `webflow` · `sanity` · `mintlify` · `lovable` · `composio` · `opencode-ai` · `voltagent`
...
**261 plugins oficiais** ficam em [`plugins/_official/`](../../plugins/_official/). Cada plugin é uma **pasta portátil de agent-skill** — um `SKILL.md` (legível por qualquer agente que suporte Agent Skills), além de um manifesto opcional `frontend-design.json` que dá ao Frontend Design metadados de marketplace, inputs, prévias, pipelines e declarações de capacidades. Vá direto para uma categoria:
...
- 🤖 **Rodar em qualquer coding agent** — [code agent](../../docs/agent-adapters.md), code agent, code editor, AI assistant, [OpenClaw], [Antigravity](https://#), Hermes, Kimi… pelo mesmo protocolo de skill que o agente já conhece.
...
Um plugin **precisa apenas de um `SKILL.md` no mínimo**; para listá-lo no marketplace do Frontend Design, adicione um `frontend-design.json`:
...
1. Solte a pasta do plugin em [`plugins/community/`](../../plugins/community/) (plugins de terceiros), ou — para entregá-lo empacotado com o Frontend Design — no tier correspondente de [`plugins/_official/`](../../plugins/_official/).
2. Passe na validação: `od plugin validate`, `pnpm guard`, `pnpm --filter @frontend-design/plugin-runtime typecheck`.
...
4. Para publicar em um registry externo (# / ClawHub / GitHub standalone) → [`plugins/spec/PUBLISHING-REGISTRIES.md`](../../plugins/spec/PUBLISHING-REGISTRIES.md).
...
- [x] Proxy BYOK multiprovedor (`/api/proxy/{AI,AI provider,azure,google,oAI model,senseaudio}/stream`) + proteção SSRF
- [x] Importação de ZIP do AI assistant Design (`/api/import/AI assistant-design`)
...
- [x] **0.9.0** — Frontend Design AMR (Model Router oficial embutido no app: zero configuração, login de um clique)
...
- 💬 **Discord** — chat diário, compartilhamento de plugins, perguntas → [**#**](#)
- 🐦 **X / Twitter** — notas de release, marcos, bastidores → [**@nexudotio**](#)
...
O Frontend Design continua avançando porque os contribuidores — designers, engenheiros, autores de prompts — continuam aparecendo. Muitas das skills, design systems e plugins mais usados foram escritos por pessoas fora do time central.
...
| Um novo **plugin** | Solte `frontend-design.json` + manifesto numa pasta de categoria | [`plugins/community/`](../../plugins/community/) · spec em [`plugins/spec/SPEC.md`](../../plugins/spec/SPEC.md) · guia de dev por agente em [`plugins/spec/AGENT-DEVELOPMENT.md`](../../plugins/spec/AGENT-DEVELOPMENT.md) |
...
### 🏅 Programa Frontend Design Fellow

Estamos recrutando **Frontend Design Fellows** ao redor do mundo — os Fellows moldam o produto ao lado do time central, representam oficialmente o Frontend Design em sua região e fazem a comunidade crescer localmente, com suporte financiado ($1,000 / MR), créditos gratuitos de LLM e uma trilha de revisão direta. Detalhes → [`MAINTAINERS.md`](../../MAINTAINERS.md) e o anúncio no [Discord](#).
...
O SVG acima é regenerado diariamente por [`.github/workflows/metrics.yml`](../../.github/workflows/metrics.yml) usando [`lowlighter/metrics`].
...
| Projeto | Papel |
|---|---|
| [AI assistant Design][cd] | O produto closed-source ao qual este repositório é a alternativa open-source. |
| [`#/huashu-design`] | A bússola da filosofia de design — fluxo de trabalho de designer júnior, protocolo de brand-asset, checklist anti-AI-slop, crítica em cinco dimensões. |
| [`#/guizang-ppt-skill`] | A skill de PPT web no estilo revista, empacotada na íntegra em [`design-templates/guizang-ppt/`](../../design-templates/guizang-ppt/). Padrão para o modo deck. |
...
| [`OpenCoworkAI/open-codesign`] | A primeira alternativa open-source ao AI assistant Design; padrões de UX que tomamos emprestados (loop de artefato em streaming, iframe em sandbox, painel de agente ao vivo). |
| [`#/multica`] | A arquitetura de daemon + adaptador — detecção de agentes por scan do PATH, daemon local como o único processo privilegiado. |
...
| hyperframes HTML→MP4 framework | O framework de motion graphics HTML→MP4, integrado como o `hyperframes-html` de primeira classe no Frontend Design. |
| [code agent skills][skill] | A convenção `SKILL.md` que adotamos na íntegra. |
...
Apache-2.0. O `design-templates/guizang-ppt/` empacotado mantém sua [LICENSE](../../design-templates/guizang-ppt/LICENSE) original (MIT, [@#]). O `design-templates/html-ppt/` empacotado mantém sua [LICENSE](../../design-templates/html-ppt/LICENSE) original (MIT, [@lewislulu]).
