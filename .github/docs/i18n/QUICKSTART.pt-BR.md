...
- **CLI de agente local (opcional):** code agent, code agent, Devin for Terminal, AI assistant CLI, OpenCode, code editor Agent, Qwen, GitHub AI assistant CLI etc. Sem nenhum instalado, use o modo BYOK API em Settings.
...
No primeiro carregamento, o app detecta o CLI de agente instalado (code agent / code agent / Devin for Terminal / AI assistant / OpenCode / code editor Agent / Qwen), seleciona automaticamente e usa por padrão o skill `web-prototype` + design system `Neutral Modern`. Digite um prompt e clique em **Send**. O agente faz streaming no painel da esquerda; a tag `<artifact>` é parseada e o HTML é renderizado ao vivo na direita. Ao terminar, clique em **Save to disk** para persistir o artifact em `./.od/artifacts/<timestamp>-<slug>/index.html`.

O dropdown **Design system** vem com **129 design systems** — 2 starters escritos à mão (Neutral Modern, Warm Editorial), 70 sistemas de produto bundled e 57 design skills vindos de [`awesome-design-skills`]. Escolha um para vestir cada protótipo na estética daquela marca.
...
pnpm --filter @frontend-design/daemon build  # build apps/daemon/dist/cli.js for `od`
pnpm --filter @frontend-design/web build     # build do pacote web quando necessário
...
Execute o Frontend Design em um ambiente totalmente conteinerizado sem instalar Node.js ou pnpm localmente.
...
O Frontend Design armazena projetos e dados SQLite dentro de um volume Docker:
...
Em seguida, abra o projeto pelo app Frontend Design novamente em vez de retomar uma sessão antiga de agente no terminal. Um agente spawnado pelo daemon deve ver valores como:
...
| **API mode** (fallback / sem CLI) | "AI API" / "AI provider API" / "Azure AI provider" / "cloud AI assistant" | Frontend → daemon `/api/proxy/{provider}/stream` → SSE do provider normalizado para `delta/end/error` → parser de artifact → preview |
...
│   │       ├── agents.ts          # PATH scanner for AI assistant/code agent/devin/AI assistant/opencode/code editor-agent/qwen/AI assistant
...
- **"no agents found on PATH"** — instale um destes: `AI assistant`, `code agent`, `devin`, `AI assistant`, `opencode`, `code editor-agent`, `qwen`, `AI assistant`. Ou troque para o modo API em Settings e cole uma chave de provider.
...
- **geração de mídia diz que `OD_BIN` está faltando ou que a URL do daemon é `:0`** — rode as verificações do dispatcher de mídia acima. Não retome a sessão antiga do CLI; reabra o projeto pelo app Frontend Design para o daemon injetar variáveis `OD_*` novas.
- **code agent carrega muito contexto de plugin** — suba o Frontend Design com `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev` para que processos code agent spawnados pelo daemon rodem com `--disable plugins`.
