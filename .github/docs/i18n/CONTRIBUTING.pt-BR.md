# Contribuindo com o Frontend Design
...
Se você não tem certeza em qual balde sua ideia se encaixa, [abra primeiro uma discussion / issue] e te apontamos para a superfície certa.
...
Você não precisa de nenhum CLI de agente no `PATH` para desenvolver o próprio OD — o daemon dirá "no agents found" e cairá no caminho **AI API · BYOK**, que é o loop de dev mais rápido de qualquer jeito.

---

## Adicionando uma nova Skill

Uma skill é uma pasta sob [`skills/`](../../skills/) com um `SKILL.md` na raiz, seguindo a [convenção `SKILL.md`][skill] do code agent mais nossa extensão opcional `od:`. **Não há passo de registro.** Coloque a pasta, reinicie o daemon e o picker mostra.
...
As três primeiras chaves são a base spec do code agent — `name`, `description`, `triggers`. Tudo sob `od:` é específico do OD e opcional, mas **`od.mode`** decide em qual grupo a skill aparece (Prototype / Deck / Template / Design system).
...
- Modo deck: [`skills/guizang-ppt/`](../../skills/guizang-ppt/) (bundled literalmente de [#/guizang-ppt-skill][guizang]) e [`skills/simple-deck/`](../../skills/simple-deck/)
...
5. **Slug em ASCII** — `#` vira `linear-app`, `x.ai` vira `x-ai`. Os 69 sistemas importados já seguem essa convenção; espelhe.
...
```javascript
{
  ...
  streamFormat: 'plain',           // or 'AI assistant-stream-json' if it speaks that
}
```

É só isso — o daemon detecta no `PATH`, o picker mostra, o caminho de chat funciona. Se o CLI emite **eventos tipados** (como o `--output-format stream-json` do code agent), conecte um parser em [`apps/daemon/src/AI assistant-stream.ts`](../../apps/daemon/src/AI assistant-stream.ts) e defina `streamFormat: 'AI assistant-stream-json'`.
...
- Pergunta de arquitetura, pergunta de design, "isso é bug ou mau uso" → [GitHub Discussions] (preferido — pesquisável para o próximo).
...
- **Embutam um runtime de modelo.** Toda a aposta do OD é "seu CLI existente já basta". Não despachamos `AI runtime`, chaves AI provider nem loaders de modelo.
...
Ao contribuir, você concorda que sua contribuição é licenciada sob a [Licença Apache-2.0](../../LICENSE) deste repositório, com a exceção dos arquivos dentro de [`skills/guizang-ppt/`](../../skills/guizang-ppt/), que mantêm sua licença MIT original e atribuição de autoria a [#].
