...
- **可選的本地 agent CLI：** code agent、code agent、Devin for Terminal、AI assistant CLI、OpenCode、code editor Agent、Qwen、Qoder CLI、GitHub AI assistant CLI 等。即使未安裝任何 CLI，也可在 Settings 中切換至 BYOK API 模式。
...
首次載入時，應用程式會掃描已安裝的 code-agent CLI（code agent / code agent / Devin for Terminal / AI assistant / OpenCode / code editor Agent / Qwen / Qoder CLI），並自動選擇其中之一；預設使用 `web-prototype` skill 與 `Neutral Modern` design system。輸入 prompt，點擊 **Send**。Agent 將以串流方式輸出至左側面板；`<artifact>` 標籤會被解析，HTML 在右側即時渲染。執行完成後，點擊 **Save to disk**，artifact 將被寫入磁碟 `./.od/artifacts/<timestamp>-<slug>/index.html`。

**Design system** 下拉選單內建 **129 套 design system** —— 包含 2 套手工編寫的 starter（Neutral Modern、Warm Editorial）、70 套打包的產品級系統，以及來自 [`awesome-design-skills`] 的 57 個 design skill。選擇任意一套，所有原型都會應用該品牌的視覺風格。
...
- **Deck / PPT** —— `simple-deck`（單檔案橫向翻頁）與 `magazine-web-ppt`（`guizang-ppt` 捆綁包，來自 [`#/guizang-ppt-skill`] —— deck mode 的預設 skill，自帶 assets/template 與 4 份 reference）。附帶 sidefile 的 skill 會自動添加一段 "Skill root (absolute)" frontmatter，使 agent 能夠基於真實的磁碟路徑解析 `assets/template.html` 與 `references/*.md`，而非在自身 CWD 中猜測。
...
pnpm --filter @frontend-design/daemon build  # 建置 apps/daemon/dist/cli.js，供 `od` 使用
pnpm --filter @frontend-design/web build     # 在需要時建置 web package
...
在一個完全容器化的環境中執行 Frontend Design，無需安裝 Node.js 或 pnpm。
...
Frontend Design 將專案和 SQLite 資料儲存在 Docker 卷中：
...
隨後，在 Frontend Design 應用程式中**重新開啟**該專案，請勿重複使用先前 terminal 中的 agent 會話。由 daemon 啟動的 agent 應當能夠看到類似如下的值：
...
| **API 模式**（fallback / 未安裝 CLI） | "AI API" / "AI provider API" / "Azure AI provider" / "cloud AI assistant" | 前端 → daemon `/api/proxy/{provider}/stream` → provider SSE 歸一化為 `delta/end/error` → artifact 解析器 → 預覽 |
...
│   │       ├── cli.ts             # `od` bin 入口
│   │       ├── server.ts          # /api/* + 靜態資源
│   │       ├── agents.ts          # 掃描 PATH 中的 AI assistant/code agent/devin/AI assistant/opencode/code editor-agent/qwen/qoder/AI assistant
...
│   ├── sidecar-proto/         # Frontend Design sidecar 協定契約
...
├── skills/                    # SKILL.md —— 任何 code agent skill 程式碼庫均可直接放入
...
├── design-systems/            # DESIGN.md —— 9 段式 schema（awesome-AI assistant-design）
...
- **"no agents found on PATH"** —— 安裝以下 CLI 之一：`AI assistant`、`code agent`、`devin`、`AI assistant`、`opencode`、`code editor-agent`、`qwen`、`qodercli`、`AI assistant`。或者在 Settings 中切換至 API mode，填入 provider key。
...
- **媒體生成發生錯誤，`OD_BIN` 缺失、或 daemon URL 為 `:0`** —— 執行上述媒體 dispatcher 問題排除步驟。請勿重複使用既有的 CLI 會話；從 Frontend Design 應用程式中重新開啟專案，daemon 才會注入新的 `OD_*` 變數。
- **code agent 載入的插件上下文過多** —— 使用 `OD_code agent_DISABLE_PLUGINS=1 pnpm tools-dev` 啟動 Frontend Design，daemon 啟動 code agent 時會傳入 `--disable plugins`。
