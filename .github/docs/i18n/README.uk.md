<h1 align="center">Frontend Design: відкрита альтернатива AI assistant Design</h1>

> 🔥 **Frontend Design 0.10.0 вже тут: універсальний Agentic-робочий простір для дизайну.** Усе ремесло тепер живе в одному вікні — від розмитої ідеї до пошуку референсів, збирання матеріалу, інтерактивного редагування, черги коментарів, шліфування анімацій і передачі результату редактору чи Code Agent — не виходячи із застосунку. З паралельними сесіями це вже не один асистент, а ціла локальна команда дизайнерів, що працює на вас. [Завантажити 0.10.0] · [Долучитися до обговорення]
>
> ⚡ **Frontend Design AMR (Agentic Model Router) — офіційний сервіс моделей.** Одне поповнення — і AI, AI assistant, AI assistant та AI model доступні просто в Frontend Design: 20+ флагманських моделей, нуль налаштувань, оплата за реальне використання токенів. [Спробувати AMR](#)
>
> 🏅 **Програма Frontend Design Fellow тепер відкрита.** Якщо ви теж вірите, що дизайн має бути відкритим — станьте Frontend Design Fellow, формуйте продукт разом із основною командою та допомагайте більшій кількості людей брати участь у визначенні майбутнього дизайну. Деталі → [`MAINTAINERS.md`](../../MAINTAINERS.md) та [Discord](#).
...
## Що таке Frontend Design

🎨 **Локально-орієнтована альтернатива [AI assistant Design][cd] з відкритим кодом.** &nbsp;🖥️ **Нативний десктопний застосунок для macOS і Windows.** &nbsp;⚡ **100+ навичок** · ✨ **150 систем `DESIGN.md` брендового рівня** · 📦 **261 готовий до використання плагін.** &nbsp;🖼️ Генерує **вебпрототипи · десктопні · мобільні прототипи**, **живі дашборди / артефакти**, **презентації**, **зображення**, **відео**, а також моушн-графіку **HyperFrames**. 🔒 Попередній перегляд у пісочниці iframe · експорт у HTML / PDF / PPTX / MP4. &nbsp;🤖 **Працює на code agent · OpenClaw · code agent · code editor · OpenCode · Qwen · AI assistant · Hermes · Kimi · Antigravity і 21 локальному CLI**, або на будь-якій AI provider-сумісній кінцевій точці через BYOK.

Frontend Design — це те, що ви отримуєте, коли **agent-native** цикл, який AI випустила разом із AI assistant Design — з'ясуй бриф, зафіксуй напрям, передавай артефакт потоком, критикуй, доставляй — перестає бути закритим і стає **файловою системою навичок, дизайн-систем і плагінів**, яку кодувальні агенти, що вже є на вашому ноутбуці, можуть читати, записувати та реміксувати. Ваш CLI стає рушієм дизайну, ваш ноутбук стає студією, а `DESIGN.md` вашої команди стає контрактом бренду.
...
Швидкий погляд на те, що таке Frontend Design і що він робить. Почніть з **Home**, оркеструйте повторювані робочі процеси за допомогою **Automation**, виокремте контракт бренду в **Design System** та розширюйте можливості за допомогою **Plugins** і **integrations**; усередині **Studio** будь-якого проєкту та сама дизайн-система передає потоком прототипи, живі артефакти, HyperFrames, презентації та зображення.
...
<td width="50%" valign="top">
<sub><b>Integrations</b> — підключайте зовнішні системи та інструменти MCP й використовуйте Frontend Design з будь-якого IDE, скрипта чи автоматизації.</sub>
</td>
</tr>
</table>
...
> Frontend Design постачається як **навички, CLI та сервер MCP**, які основні кодувальні агенти споживають нативно. Щойно OD встановлено, одна команда `od mcp install <agent>` під'єднує сервер MCP до конфігурації цього агента, і ви викликаєте ті самі інструменти зсередини будь-якого агента.

| Кодувальний агент / платформа &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | Статус &nbsp;&nbsp; | Встановлення сервера MCP одним рядком &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|---|:---:|---|
| [code agent](design documentation assistant-code) | ✅ Підтримується | `od mcp install AI assistant` |
| [code agent CLI] | ✅ Підтримується | `od mcp install code agent` |
| [code editor](# editor.com/cli) | ✅ Підтримується | `od mcp install code editor` |
| [VS Code + GitHub AI assistant] | ✅ Підтримується | `od mcp install AI assistant` |
| [GitHub AI assistant CLI] | ✅ Підтримується | `od mcp install AI assistant` |
| [AI assistant CLI] | ✅ Підтримується | `od mcp install AI assistant` |
| [OpenCode](#) | ✅ Підтримується | `od mcp install opencode` |
| [OpenClaw] | ✅ Підтримується | `od mcp install openclaw` |
| [Antigravity](https://#) | ✅ Підтримується | `od mcp install antigravity` |
| [Cline] | ✅ Підтримується | `od mcp install cline` |
| [Trae](https://#/) | ✅ Підтримується | `od mcp install trae` |
| [Kimi CLI] | ✅ Підтримується | `od mcp install kimi` |
| [Pi Agent] | ✅ Підтримується | `od mcp install pi` |
| [AI provider Vibe CLI] | ✅ Підтримується | `od mcp install vibe` |
...
**Не встановлено жодного CLI?** Проксі BYOK на `POST /api/proxy/{AI,AI provider,azure,google,oAI model,senseaudio}/stream` дає вам той самий цикл (без породження процесу) — вставте `baseUrl` + `apiKey` + `model`, з підтримкою AI provider, AI, Azure AI provider, cloud AI assistant, OAI model, LM Studio, vLLM або будь-якої AI provider-сумісної кінцевої точки. Захист від SSRF для кожної цілі блокує внутрішні IP / link-local / CGNAT на межі демона.
...
<sub><b>Прототип мобільного застосунку</b> — гейміфікований потік із трьох екранів зі стрічками XP та деталями квестів. Передавайте прямо в code editor / code agent / code agent, щоб перетворити на React/Next/Vue.</sub>
</td>
</tr>
</table>
...
<sub><b>Режим презентації (guizang-ppt)</b> — журнальні макети, WebGL-герой, чеклісти P0/P1/P2. Включено дослівно з <a href=" зі збереженням оригінальної ліцензії.</sub>
</td>
<td width="50%" valign="top">
...
### 4 · Зображення — `AI-image-2`, ImageRouter, користувацький API

<table>
<tr>
<td width="20%" valign="top"><img src="#" alt="Illustrated city food map" /><br/><sub><b>Ілюстрована мапа міської їжі</b><br/>Намальований від руки редакторський тревел-постер</sub></td>
<td width="20%" valign="top"><img src="#" alt="Cinematic elevator scene" /><br/><sub><b>Кінематографічна сцена в ліфті</b><br/>Редакторський однокадровий стоп-кадр</sub></td>
<td width="20%" valign="top"><img src="#" alt="Cyberpunk anime portrait" /><br/><sub><b>Кіберпанк-портрет</b><br/>Аватар профілю — неоновий текст на обличчі</sub></td>
<td width="20%" valign="top"><img src="#" alt="3D stone staircase evolution" /><br/><sub><b>3D кам'яні сходи</b><br/>Інфографіка з тесаного каменю</sub></td>
<td width="20%" valign="top"><img src="#" alt="Glamorous portrait" /><br/><sub><b>Гламурний портрет</b><br/>Редакторський студійний знімок</sub></td>
</tr>
</table>
...
**[HyperFrames][hyperframes]** — це відкрита, agent-native відеоплатформа від HeyGen, інтегрована як повноправний учасник в Frontend Design. Агент пише HTML + CSS + GSAP, а HyperFrames рендерить це в детермінований MP4 через headless Chrome + FFmpeg. Поєднайте її з **Seedance 2.0** для кінематографічного t2v / i2v, **Veo 3 / Sora 2 / Kling 2** для маршрутизованих варіантів моделей та **Suno v5 / Lyria 2** для аудіошару.

<table>
<tr>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-saas-product-promo-30s.json"><img src="#" alt="SaaS promo" /></a><br/><sub><b>30-секундне промо SaaS-продукту</b> · 16:9 · 3D-розкриття UI</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-tiktok-karaoke-talking-head.json"><img src="#" alt="TikTok karaoke" /></a><br/><sub><b>TikTok-караоке з говорильною головою</b> · 9:16 · TTS + синхронізовані за словами субтитри</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-brand-sizzle-reel.json"><img src="#" alt="Brand sizzle reel" /></a><br/><sub><b>30-секундний брендовий sizzle-ролик</b> · 16:9 · аудіореактивна кінетична типографіка</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-data-bar-chart-race.json"><img src="#" alt="Bar chart race" /></a><br/><sub><b>Гонка стовпчикових діаграм</b> · 16:9 · інфографіка даних у стилі NYT</sub></td>
</tr>
<tr>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-flight-map-route.json"><img src="#" alt="Flight map" /></a><br/><sub><b>Мапа польоту</b> · 16:9 · розкриття маршруту в стилі Apple</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-logo-outro-cinematic.json"><img src="#" alt="Logo outro" /></a><br/><sub><b>4-секундна кінематографічна заставка логотипа</b> · 16:9 · поелементне складання + bloom</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-money-counter-hype.json"><img src="#" alt="Money counter" /></a><br/><sub><b>Лічильник грошей $0 → $10K</b> · 9:16 · хайп у стилі Apple</sub></td>
<td width="25%" valign="top"><a href="../../prompt-templates/video/hyperframes-website-to-video-promo.json"><img src="#" alt="Website to video" /></a><br/><sub><b>Сайт-у-відео</b> · 16:9 · захоплює сайт у 3 вікнах перегляду</sub></td>
</tr>
</table>
...
## Чому Frontend Design

> **У квітні 2026 року AI випустила [AI assistant Design][cd] — уперше LLM перестала писати прозу й почала доставляти дизайн-артефакти безпосередньо.** Це стало вірусним. Але вона залишилася закритою, лише платною, лише хмарною, прив'язаною до моделі AI, навичок AI, поверхні AI. Без виходу, без самохостингу, без розгортання на Vercel, без можливості підставити власного агента.

Frontend Design (OD) — це альтернатива з відкритим кодом. Той самий цикл, та сама ментальна модель «спершу артефакт», без жодної прив'язки:

- 🤖 **Agent-native, незалежна від моделі.** Ми не постачаємо агента. `AI assistant` / `code agent` / `code editor-agent` / `AI assistant` / `hermes` / `kimi`, які вже є у вашому `PATH`, і є рушієм дизайну. Замінюйте одним кліком.
- 🧠 **Брендовий рівень за замовчуванням.** Кожен рендер зчитує активний `DESIGN.md` — схему з 9 розділів, що охоплює палітру, типографіку, відступи, рух, голос, антипатерни. 150 систем постачаються разом із репозиторієм (Linear, Stripe, Vercel, Airbnb, Apple, Tesla, Notion, AI, code editor, Supabase, Figma…). Покладіть теку всередину — селектор її знайде.
...
| Agent-native (працює у вашому CLI) | Лише AI | ❌ | Лише хмарний агент | **✅ 21 CLI + BYOK** |
...
Найшвидший спосіб скористатися Frontend Design. Без Node, без pnpm, без клонування.

- **macOS** (Apple Silicon · Intel x64) → [**#**](#) або [GitHub Releases]
- **Windows** (x64) → [**#**](#) або [GitHub Releases]
...
Ви можете користуватися Frontend Design, жодного разу не відкриваючи GUI — викликайте його як навичку, плагін або сервер MCP усередині code agent, code agent, code editor, AI assistant, OpenClaw, Antigravity, Hermes, Kimi тощо.
...
4. **Передача в інженерію.** Артефакт — це справжній HTML/CSS — скиньте його в code editor, code agent або code agent, щоб продовжувати будувати як код. Або експортуйте PPTX / PDF / MP4 прямо в маркетинг.
5. **Frontend Design стає розумнішим у міру використання.** Ваші скриншоти, шрифти, палітри та підтверджені артефакти накопичуються як значення за замовчуванням для наступної сесії. Менше переробки, менше дрейфу.

---

## Використання Frontend Design зі свого кодувального агента

Frontend Design постачає **сервер MCP через stdio** та **скрипти встановлення** для кожного агента. Будь-який MCP-сумісний агент в іншому репозиторії може читати файли з ваших локальних проєктів Frontend Design безпосередньо — CSS токенів, JSX-компоненти, вхідний HTML — як структурований API, до якого можна звертатися за іменем. Агент завжди бачить живий файл, а не застарілий експорт.
...
**Для агента, що починає з нуля,** інсталятор розміщує `~/.config/<agent>/frontend-design.json` (або платформенний еквівалент) плюс готовий до вставлення фрагмент MCP. code editor отримує deeplink в один клік; code agent отримує однорядкову команду `AI assistant mcp add-json`; кожен інший агент отримує JSON у схемі, якої очікує його конфігурація. Повний процес для кожного агента → **Settings → tool server** у десктопному застосунку або [`docs/agent-adapters.md`](../../docs/agent-adapters.md).
...
**AI та LLM** — `AI assistant` · `cohere` · `AI provider-ai` · `minimax` · `together-ai` · `AI platform` · `runwayml` · `elevenlabs` · `oAI model` · `x-ai`

**Інструменти розробника** — `code editor` · `vercel` · `linear-app` · `framer` · `expo` · `clickhouse` · `mongodb` · `supabase` · `hashicorp` · `posthog` · `sentry` · `warp` · `webflow` · `sanity` · `mintlify` · `lovable` · `composio` · `opencode-ai` · `voltagent`
...
**261 офіційний плагін** міститься в [`plugins/_official/`](../../plugins/_official/). Кожен плагін — це **портативна тека agent-skill** — `SKILL.md` (зчитувана будь-яким агентом, що підтримує Agent Skills), плюс опціональний маніфест `frontend-design.json`, який надає Frontend Design метадані маркетплейсу, входи, попередні перегляди, конвеєри та декларації можливостей. Перейдіть одразу до категорії:
...
- 🤖 **Працюють у будь-якому кодувальному агенті** — [code agent](../../docs/agent-adapters.md), code agent, code editor, AI assistant, [OpenClaw], [Antigravity](https://#), Hermes, Kimi… через той самий протокол навичок, який агент уже знає.
...
Плагіну **щонайменше потрібен лише `SKILL.md`**; щоб помістити його в маркетплейс Frontend Design, додайте `frontend-design.json`:
...
1. Покладіть теку плагіна в [`plugins/community/`](../../plugins/community/) (сторонні плагіни), або — щоб постачати його в комплекті з Frontend Design — у відповідний рівень [`plugins/_official/`](../../plugins/_official/).
2. Пройдіть валідацію: `od plugin validate`, `pnpm guard`, `pnpm --filter @frontend-design/plugin-runtime typecheck`.
...
4. Щоб опублікувати у зовнішньому реєстрі (# / ClawHub / окремий GitHub) → [`plugins/spec/PUBLISHING-REGISTRIES.md`](../../plugins/spec/PUBLISHING-REGISTRIES.md).
...
- [x] Мультипровайдерний проксі BYOK (`/api/proxy/{AI,AI provider,azure,google,oAI model,senseaudio}/stream`) + захист від SSRF
- [x] Імпорт ZIP AI assistant Design (`/api/import/AI assistant-design`)
...
- [x] **0.9.0** — Frontend Design AMR (офіційний Model Router, вбудований у застосунок: нуль конфігурації, вхід в один клік)
...
- 💬 **Discord** — щоденний чат, обмін плагінами, запитання → [**#**](#)
...
Frontend Design продовжує рухатися, бо контриб'ютори — дизайнери, інженери, автори промптів — продовжують приходити. Багато з найуживаніших навичок, дизайн-систем і плагінів написали люди поза основною командою.
...
| Новий **плагін** | Покладіть `frontend-design.json` + маніфест під текою категорії | [`plugins/community/`](../../plugins/community/) · специфікація в [`plugins/spec/SPEC.md`](../../plugins/spec/SPEC.md) · посібник із розробки агентом у [`plugins/spec/AGENT-DEVELOPMENT.md`](../../plugins/spec/AGENT-DEVELOPMENT.md) |
...
### 🏅 Програма Frontend Design Fellow

Ми набираємо **Frontend Design Fellows** по всьому світу — Fellows формують продукт разом із основною командою, офіційно представляють Frontend Design у своєму регіоні та розвивають спільноту локально, маючи фінансовану підтримку ($1,000 / MR), безкоштовні LLM-кредити та прямий трек рев'ю. Деталі → [`MAINTAINERS.md`](../../MAINTAINERS.md) та анонс у [Discord](#).
...
SVG вище щодня перегенеровується [`.github/workflows/metrics.yml`](../../.github/workflows/metrics.yml) за допомогою [`lowlighter/metrics`].
...
| Проєкт | Роль |
|---|---|
| [AI assistant Design][cd] | Закритий продукт, відкритою альтернативою якому є цей репозиторій. |
| [`#/huashu-design`] | Компас дизайн-філософії — робочий процес джуніор-дизайнера, протокол брендових ресурсів, чекліст проти AI-slop, п'ятивимірна критика. |
| [`#/guizang-ppt-skill`] | Навичка веб-PPT у журнальному стилі, включена дослівно під [`design-templates/guizang-ppt/`](../../design-templates/guizang-ppt/). Типова для режиму презентацій. |
...
| [`OpenCoworkAI/open-codesign`] | Перша відкрита альтернатива AI assistant Design; UX-патерни, які ми запозичуємо (цикл потокового артефакта, iframe-пісочниця, жива панель агента). |
| [`#/multica`] | Архітектура демон + адаптер — виявлення агентів скануванням PATH, локальний демон як єдиний привілейований процес. |
...
| hyperframes HTML→MP4 framework | Платформа моушн-графіки HTML→MP4, інтегрована як повноправний `hyperframes-html` в Frontend Design. |
| [code agent skills][skill] | Конвенція `SKILL.md`, яку ми приймаємо дослівно. |
...
Apache-2.0. Включений `design-templates/guizang-ppt/` зберігає свою оригінальну [LICENSE](../../design-templates/guizang-ppt/LICENSE) (MIT, [@#]). Включений `design-templates/html-ppt/` зберігає свою оригінальну [LICENSE](../../design-templates/html-ppt/LICENSE) (MIT, [@lewislulu]).
