...
description: |
  ...
  Frontend Design browser-tab context, testing web apps, dogfooding Frontend Design
  previews, QA, bug hunts, or reviewing app quality. Prefer local Frontend Design
...
Use `agent-browser` for local Frontend Design preview validation: inspect rendered
...
For selected Frontend Design browser tabs and browser-use/browser-harness-style
...
5. If the selected Frontend Design context only provided a URL/title and no browser
...
Do not run Frontend Design's own daemon CLI as a browser automation tool. Commands
...
With the Frontend Design preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `Frontend Design`, current URL under `127.0.0.1:17573`,
visible Frontend Design UI text in the snapshot, and a screenshot at
