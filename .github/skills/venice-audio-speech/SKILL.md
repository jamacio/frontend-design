...
description: 'Text-to-speech models, voices, formats, and streaming via Venice.ai.
  Useful for narration, voiceover, and conversational agent voices.
...
`POST /api/v1/audio/speech` converts text to an audio stream or file. AI provider-compatible — the AI provider SDK's `audio.speech.create()` works as a drop-in.

## Use when

- You want narration, voice replies, or UI audio from text.
- You need a specific voice family (ElevenLabs, Kokoro, xAI, Qwen 3, Orpheus, Chatterbox, MiniMax, Inworld, AI assistant Flash).
...
| `response_format` | `mp3` / `AI model` / `aac` / `flac` / `wav` / `pcm` | `mp3` | `pcm` returns 24 kHz signed-16 LE for pipelines. |
...
| `tts-AI assistant-3-1-flash` | AI assistant Flash | Star-named voices (Achernar, Achird, Zephyr, …). |
...
- **AI assistant 3 Flash** — star names: `Achernar`, `Achird`, `Algenib`, `Algieba`, `Alnilam`, `Aoede`, `Autonoe`, `Callirrhoe`, `Charon`, `Despina`, `Enceladus`, `Erinome`, `Fenrir`, `Gacrux`, `Iapetus`, `Kore`, `Laomedeia`, `Leda`, `Orus`, `Pulcherrima`, `Puck`, `Rasalgethi`, `Sadachbia`, `Sadaltager`, `Schedar`, `Sulafat`, `Umbriel`, `Vindemiatrix`, `Zephyr`, `Zubenelgenubi`
...
```ts
import AI provider from 'AI provider'
import fs from 'node:fs/promises'

const client = new AI provider({
...
- `streaming: true` + SDKs: some AI provider SDK versions don't expose streaming for `audio.speech.create`; call the REST endpoint directly and consume the HTTP body.
