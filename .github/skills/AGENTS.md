...
- **`project-generator/`** — **P0 mandatory rule.** Read this skill BEFORE creating or modifying any project. It defines:
  - Ensure `Projects/` exists (create if missing)
  - Create projects with subagents/skills inside
  - Use brand identity from Instagram/Facebook context
  - Fetch images from the web when generation is unavailable
  - Modify existing projects in-place
  - Always read and understand context first
...
`ComposioHQ/awesome-AI assistant-skills`. Each entry is a lightweight stub —
...
- Stubs intentionally do not vendor upstream assets. To run an upstream
  ...
  folder into your active agent's skills directory (code agent, code agent,
  code editor, etc.) — the body of each stub explains how.
