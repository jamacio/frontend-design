...
name: image-to-code
description: |
  Elite website image-to-code skill for code agent. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. In code agent, it must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
triggers:
  - "image to code"
...
- In code agent, bias strongly toward larger, more analyzable section images.
...
## 4. code agent-SPECIFIC SECTION IMAGE RULE

Inside code agent, do not compress too many website sections into one single image if that would make the text, spacing, buttons, or layout details too small to analyze properly.

In code agent, prefer separate large images per section.

Default rule inside code agent:
...
Outside code agent, this skill may still allow more compact multi-section composition when appropriate.
Inside code agent, prioritize section clarity and extraction accuracy.
...
## 10. IMAGE-FIRST code agent WEBSITE WORKFLOW

When this skill is used inside code agent or any environment that supports image generation plus implementation, default to an image-first workflow for website design tasks.
...
Inside code agent, treat each section as its own analyzable unit.
...
In code agent, these should usually become section-by-section images, not one compressed sheet.
...
17. Has code agent avoided compressing too many sections into one tiny image?
...
4. inside code agent, prefer one large image per section
...
Do not compress many sections into one unreadable image in code agent.
...
Interpretation:
- generate 8 separate section images in code agent
- one per section
...
Interpretation:
- generate 4 separate section images in code agent
...
Inside code agent, if the user wants multiple sections, prefer separate large section images instead of one compressed multi-section board, so text, spacing, typography, buttons, and colors can be extracted properly.
