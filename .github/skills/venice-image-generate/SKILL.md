...
2. **`POST /api/v1/images/generations`** — AI provider-compatible, fewer knobs but drop-in for the AI provider SDK.
...
- You're porting from AI provider's `images.generate` and want a zero-change SDK swap.
...
## `/images/generations` — AI provider-compatible

Use this if you're already on the AI provider SDK. Field names match `AI provider.images.generate()`.
...
| `quality`, `style` (`vivid`/`natural`), `background`, `output_compression`, `user` | — | Accepted for AI provider compat, not used by Venice. |
...
- Each model picks one sizing idiom: either `width`/`height`, `aspect_ratio` + `resolution`, or (AI provider-compat) `size`. Match the model's `constraints`.
...
- For AI provider-compat, `response_format: "url"` returns a **data URL**, not a hosted URL — plan for that if you're saving to storage.
