...
Companion to `layout-discipline.md`. The rail / code editor primitives in that
...
most common pairing in Frontend Design today, but the same machinery
...
The `code editor.take(gap=Inches(0.12))` default suits 14pt Latin body copy.
...
- [ ] **Beyond CJK:** RTL slides set `<a:rtl val="1"/>` on the
      ...
      code editor `gap` is bumped per the line-height table above when the
