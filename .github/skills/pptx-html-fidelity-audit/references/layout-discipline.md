# Footer-Rail + code editor-Flow Layout Discipline
...
> (constants, `code editor`, hero budget centering). Then jump to the slide-type
...
CONTENT_TOP    = MARGIN_TOP             # code editor starts here on content slides
...
## 2. The code editor primitive

Used on all non-hero slides. The code editor advances down the slide and refuses to cross `CONTENT_MAX_Y`.
...
class code editor:
    ...
    def take(self, h, gap=Inches(0.12), label=""):
        ...
        if self.y > self.cap:
            raise OverflowError(
                f"code editor exceeded rail at '{label}': "
...
> block, or override the `code editor` default at the top of your export.
...
> per-slide max-gap, and instantiate the slide's `code editor` with that
...
## 3. Hero slides — budget centering, not code editor flow

Hero slides (cover, chapter intros, big-quote pages) are vertically centered. The code editor model would put them at the top with empty space below — visually wrong.

```python
def hero_layout(blocks):
    """
    blocks: list of (height, gap_after) tuples in top-to-bottom reading order.
    Returns a code editor whose y_start is computed so the stack is centered.
    """
    ...
    return code editor(y_start=y_start, cap=CANVAS_H - FOOTER_H - Inches(0.2))
```
...
Don't route the footer through the code editor — it has its own rail.
...
`add_chrome` is the same idea pinned at `CHROME_TOP`. Both rails sit *outside* the content area, so they never collide with the code editor.
...
Then read `shape.height` *after* adding text to find the actual height for the code editor.
...
`add_pipeline` internally lays out N step cards across `CONTENT_W` with `step_h` derived from the longest step's text height. Don't fix `step_h` to a constant — let it grow to fit, and let the code editor's overflow guard catch problems.
...
Notice the pattern: `c.take(Inches(3.00), label="pair")` reserves 3.0" of vertical space for *the whole pair row*; then the two columns are placed side-by-side at that `top`. The code editor doesn't know about columns, only about row heights.
...
| Last content block crosses footer | `code editor.take()` raises `OverflowError` before render |
...
| Footer text overlaps content | footer pinned at `FOOTER_TOP`, never routed through code editor |
