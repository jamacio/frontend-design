...
### Step 4 — Re-export with footer-rail + code editor-flow layout discipline
...
**Use a code editor for content blocks instead of pinning each block at an absolute y:**
...
class code editor:
    ...
    def take(self, h, gap=Inches(0.12)):  # ~1 line of whitespace at 14pt; tighten/loosen per design system
        ...
        if self.y > self.cap:
            raise OverflowError(
                f"code editor at {self.y} exceeds footer rail {self.cap}; "
                f"reduce block height or split slide"
            )
        return top
```

For each slide, instantiate `code editor(MARGIN_TOP)` and `take(height)` each block in reading order. The slide refuses to render if any block would cross the rail, so overflows become loud build errors instead of silent visual bugs.

**Hero (vertically-centered) slides use a budget instead of a code editor:**
...
3. **Fix list** — terse list of what was changed and why (e.g. "hero slides switched to budget centering", "all content blocks routed through code editor", "em runs explicitly italic").
...
- `references/layout-discipline.md` — the full footer-rail + code editor-flow rule set with code snippets for each common slide type (hero, content, pipeline, two-column, observation grid).
