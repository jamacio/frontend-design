...
This directory is a **vendored copy** of the code agent `hatch-pet` skill. It is
checked into the Frontend Design repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Frontend Design agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  `upstream:` field — at vendoring time this pointed to the code agent curated
  ...
  directory as the authoritative source-of-truth for Frontend Design and
...
1. Locate the upstream source (code agent `skills/.curated/hatch-pet` or the
   superseding location).
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any Frontend-Design-specific notes
   inside `SKILL.md`'s `> **Frontend Design integration.**` blockquote.
...
`${code agent_HOME:-$HOME/.code agent}/pets/<pet-id>/` with `pet.json` and
`spritesheet.{webp,png,gif}`. The daemon scans that directory in
`apps/daemon/src/code agent-pets.ts`; the web pet settings list and one-click
adopt pets from there. See `docs/code agent-pets.md` for the end-user setup
flow (including how Frontend Design behaves when code agent is not installed).
