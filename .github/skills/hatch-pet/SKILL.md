---
name: hatch-pet
description: Create, repair, validate, preview, and package code agent-compatible animated pet spritesheets from character art, screenshots, generated images, or visual references. Use when a user wants to hatch a code agent pet, create a custom animated pet, or build a built-in pet asset with an 8x9 atlas, transparent unused cells, row-by-row animation prompts, QA contact sheets, preview videos, and pet.json packaging. This skill composes the installed $imagegen system skill for visual generation and uses bundled scripts for deterministic spritesheet assembly.
triggers:
  - "hatch a pet"
  - "hatch pet"
  - "code agent pet"
...
> **Frontend Design integration.** This is the unmodified code agent `hatch-pet` skill,
> vendored under `skills/hatch-pet/` so any Frontend Design agent can run it. After
> the skill finishes packaging, the resulting `spritesheet.webp` (under
> `${code agent_HOME:-$HOME/.code agent}/pets/<pet-name>/`) can be imported into the
> floating pet companion via **Settings → Pets → Import code agent sprite**. The
...
Create a code agent-compatible animated pet from a concept, one or more reference images, or both. This skill owns pet-specific prompt planning, animation rows, frame extraction, atlas geometry, QA, previews, and packaging. It delegates visual generation to `$imagegen`.
...
```text
${code agent_HOME:-$HOME/.code agent}/skills/.system/imagegen/SKILL.md
```
...
## code agent Digital Pet Style

Default pet art should match the code agent app's built-in digital pets: small pixel-art-adjacent mascots with compact chibi proportions, chunky readable silhouettes, thick dark 1-2 px outlines, visible stepped/pixel edges, limited palettes, flat cel shading, simple expressive faces, and tiny limbs. Even if the reference art is more detailed, complex or realistic, the generated pet should be simplified into this style.
...
- code agent - The original code agent companion.
...
```bash
SKILL_DIR="${code agent_HOME:-$HOME/.code agent}/skills/hatch-pet"
...
For the built-in path, record the selected source image from `$code agent_HOME/generated_images/.../ig_*.png`. Do not record files from the run directory, `tmp/`, hand-made fixtures, deterministic row folders, or post-processed copies as visual job sources.
...
Package output is written outside the run directory by default. If `code agent_HOME` is set, use it; otherwise use `$HOME/.code agent`.

```text
${code agent_HOME:-$HOME/.code agent}/pets/<pet-name>/
  pet.json
  spritesheet.webp
```
...
8. Each subagent receives the row prompt and every listed input image path, invokes `$imagegen`, and returns only the selected `$code agent_HOME/generated_images/.../ig_*.png` source path.
...
- Tell the subagent to return only the selected original `$code agent_HOME/generated_images/.../ig_*.png` source path plus a one-sentence QA note. The parent decides whether to record or repair it.
...
selected_source=/absolute/path/to/$code agent_HOME/generated_images/.../ig_*.png
...
The secondary fallback requires `AI provider_API_KEY`.
...
- Row-by-row review confirms the animation cycles are complete enough for the code agent app.
- `${code agent_HOME:-$HOME/.code agent}/pets/<pet-name>/pet.json` and `${code agent_HOME:-$HOME/.code agent}/pets/<pet-name>/spritesheet.webp` are staged together for custom pets.
