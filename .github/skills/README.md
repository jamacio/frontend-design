...
A skill is the atomic unit of design capability in Frontend Design — one folder, one `SKILL.md`, optional `assets/` and `references/`. The daemon scans this directory at startup; drop a folder in, restart, and the picker shows it.
...
Skills in this directory are Apache-2.0 unless their own `LICENSE` says otherwise. The most notable exception is [`skills/guizang-ppt/`](guizang-ppt/), bundled verbatim from [#/guizang-ppt-skill] under MIT.
