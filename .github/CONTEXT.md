...
Frontend Design is a local-first design workspace where projects contain generated design files and agent conversations. This glossary records domain language only, not implementation details.
...
**Active Project**:
The project the user most recently interacted with in the Frontend Design UI and that tool integrations may use when no project is specified.
...
**AMR Cloud**:
The user-facing cloud runtime option for Frontend Design's official model router, shown in onboarding and login-oriented product surfaces.
...
The separately owned release contract that provides the native AMR CLI builds Frontend Design can package.
_Avoid_: Frontend Design release channel, package build step, source checkout
...
_Avoid_: Frontend Design supported platforms, release channel, future platform promise
...
- The **AMR CLI Distribution Contract** is owned separately from Frontend Design; Frontend Design release packaging consumes it instead of defining the native CLI release itself.
...
> **Dev:** "When a coding agent creates a code agent deck through MCP, should it create a live artifact?"
