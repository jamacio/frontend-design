...
/**
 * frontend-design-landing-deck — slide deck composer.
 *
 ...
 * stylesheet from the sister `frontend-design-landing` skill, then layers
...
import type {
  FrontendDesignLandingDeckInputs,
  Slide,
  CoverSlide,
  SectionSlide,
  ContentSlide,
...
const SISTER_STYLES = resolve(SKILL_ROOT, '..', 'frontend-design-landing', 'styles.css');
...
/* ------------------------------------------------------------------ *
 * deck-specific stylesheet (layered on top of frontend-design-landing CSS).
 *
...
function chromeStrip(brand: FrontendDesignLandingDeckInputs['brand'], deckTitle: string): string {
...
function footStrip(idx: number, total: number, brand: FrontendDesignLandingDeckInputs['brand']): string {
...
function renderSlide(
  ...
  i: number,
  total: number,
  inputs: FrontendDesignLandingDeckInputs,
  assets: string,
): string {
...
(function () {
  ...
  function applySlide(n) {
    ...
    /* load-bearing: .slide.active is read by Frontend Design's host bridge
  ...
  /* Host-driven navigation: Frontend Design's host bridge classifies this deck
...
export function renderDeck(inputs: FrontendDesignLandingDeckInputs, baseCss: string): string {
...
async function main(): Promise<void> {
  ...
  const inputs = JSON.parse(inputsRaw) as FrontendDesignLandingDeckInputs;
