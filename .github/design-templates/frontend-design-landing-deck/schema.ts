/**
 * frontend-design-landing-deck — input schema.
 *
 * Sister skill to `frontend-design-landing`. Produces a single-file slide
...
import type { MixedText, BrandBlock, ImageryConfig } from '../frontend-design-landing/schema';
...
export interface CoverSlide {
  kind: 'cover';
  /** Eyebrow above the title — `'Frontend Design · Vol. 01'`. */
  eyebrow: string;
...
export interface EndSlide {
  kind: 'end';
  /** The huge kicker — `'Frontend Design.'`. */
  mega: MixedText;
...
export interface FrontendDesignLandingDeckInputs {
  $schema?: string;
  brand: BrandBlock;
  /** Deck-wide title shown in the HUD — `'Frontend Design · Vol. 01'`. */
  deck_title: string;
  slides: Slide[];
...
/**
 * @deprecated Use `FrontendDesignLandingDeckInputs`.
 *
 ...
 */
export type EditorialCollageDeckInputs = FrontendDesignLandingDeckInputs;
