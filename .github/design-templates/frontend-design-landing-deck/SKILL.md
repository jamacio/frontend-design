---
name: frontend-design-landing-deck
description: >
  ...
  dots, surreal collage plates) — Frontend Design's brand deck recipe.
...
triggers:
  ...
  - editorial slides
  - atelier zero deck
  - frontend design deck
  - frontend design landing deck
od:
  category: brand-deck
  surface: web
  mode: deck
...
inputs:
  ...
  - id: deck_title
    ...
    description: e.g. `'Frontend Design · Vol. 01 / Issue Nº 26'`.
  - id: slides
  ...
  - id: imagery
    ...
    schema_path: ../frontend-design-landing/schema.ts#ImageryConfig
parameters:
...
example_prompt: |
  ...
  card. Reuse the frontend-design-landing image library.
---

# frontend-design-landing-deck

Sister skill to [`frontend-design-landing`](../frontend-design-landing/). Same
...
```text
inputs.json + ../frontend-design-landing/styles.css
        │
        └──────────► scripts/compose.ts
                            │
                            ▼
...
`frontend-design-landing` brief, copy `brand` and `imagery` over verbatim.
...
script — it shares the 16-slot library from `frontend-design-landing`. To
regenerate or stub:

```bash
# generate via image-gen-2 (fal.ai)
FAL_KEY=... npx tsx ../frontend-design-landing/scripts/imagegen.ts ../frontend-design-landing/inputs.example.json --out=../frontend-design-landing/assets/

# or paper-textured SVG placeholders
npx tsx ../frontend-design-landing/scripts/placeholder.ts ../frontend-design-landing/assets/
```

Set your deck's `inputs.imagery.assets_path` to wherever those PNGs
live (default in the example: `../frontend-design-landing/assets/`).
...
stylesheet from `../frontend-design-landing/styles.css`, layers
...
- **Reuse the sister skill's stylesheet.** The composer reads
  `../frontend-design-landing/styles.css` at compile time. Do not
...
- [`frontend-design-landing`](../frontend-design-landing/) — landing page sister skill.
