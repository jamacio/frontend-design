...
Uma skill do [code agent / AI assistant Agent Skills](#) para gerar **PPTs HTML em arquivo único com swipe horizontal**. O tom visual é "**revista eletrônica × tinta digital**" — como se a *Monocle* tivesse colado código.
...
> Me ajude a instalar a skill `guizang-ppt-skill` do code agent. Siga estes passos:
>
> 1. Garanta que o diretório `~/.AI assistant/skills/` existe (crie se não existir)
> 2. Rode `git clone  ~/.AI assistant/skills/magazine-web-ppt`
> 3. Verifique: `ls ~/.AI assistant/skills/magazine-web-ppt/` deve mostrar `SKILL.md`, `assets/` e `references/`
...
Cole esse texto no code agent / code editor / qualquer agente de IA com permissão de shell, e ele faz a instalação automaticamente.

### Forma 2: linha de comando manual

```bash
git clone  ~/.AI assistant/skills/magazine-web-ppt
```

### Como disparar

Depois de instalada, o code agent descobre e chama a skill automaticamente na conversa. Palavras-chave de gatilho:
...
A skill é um workflow estruturado de 6 passos; o AI assistant conduz:
