...
一个 [code agent / AI assistant Agent Skills](#) 技能,用于生成**单文件 HTML 横向翻页 PPT**,视觉基调是"**电子杂志 × 电子墨水**"——像 *Monocle* 贴上了代码的样子。
...
> 帮我安装 `guizang-ppt-skill` 这个 code agent skill。请按下面步骤做:
>
> 1. 确保 `~/.AI assistant/skills/` 目录存在(不存在就创建)
> 2. 执行 `git clone  ~/.AI assistant/skills/magazine-web-ppt`
> 3. 验证:`ls ~/.AI assistant/skills/magazine-web-ppt/` 应该看到 `SKILL.md`、`assets/`、`references/` 三项
> 4. 告诉我安装好了,之后我说"做一份杂志风 PPT"之类的话就会触发这个 skill

把这段话复制粘贴给 code agent / code editor / 任何有 shell 权限的 AI Agent,它会自动完成安装。
...
装好后,code agent 会在对话里自动发现并调用这个 skill。触发关键词:
...
Skill 本身是结构化的 6 步工作流,AI assistant 会逐步引导:
