...
name: live-artifact
description: |
  Create refreshable, auditable Frontend Design artifacts backed by connector or local data.
...
Create an Frontend Design live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
5. **Register or update through daemon wrappers**
   - Use the Frontend Design daemon wrapper commands via `"$OD_NODE_BIN" "$OD_BIN"` instead of raw `curl`, bare `node`, or bare `od`:
