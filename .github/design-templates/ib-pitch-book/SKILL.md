...
description: |
  ...
  AI financial-services Pitch Agent (Apache-2.0).
triggers:
  - "ib pitch book"
...
of AI's **Pitch Agent** from
[`financial-services`],
repackaged as an Frontend Design `deck` skill.
...
Follow Frontend Design's deck export path for the active session (HTML / PDF /
PPTX per daemon capabilities).

## Relationship to Frontend Design financial skills
...
AI's Apache-2.0 **financial-services** repository; this skill file is an
original adaptation for Frontend Design.
