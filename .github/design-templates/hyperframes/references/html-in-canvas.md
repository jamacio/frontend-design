...
When this skill runs inside Frontend Design, the daemon shells out to `npx hyperframes render`, which inherits the auto-enable. You do **not** need to add browser flags or pass extra CLI args from the agent.
...
| Text code editor | Dramatic text reveal with chromatic shadows | `npx hyperframes add vfx-text-code editor` |
...
For Docker renders, the flag is also enabled automatically inside the container. Inside Frontend Design, the daemon's `npx hyperframes render` call (`apps/daemon/src/media.ts`) inherits the same default — you don't need to thread anything through.
