...
This pack is consumed by the `frontend-design-landing` skill. Every page-level
image is rendered with `image-gen-fal` (preferred) or `image-gen-azure`.
...
If `image-gen-fal` is unavailable, the same prompts work with
`image-gen-azure` — but mask-based inpainting is azure-only.
