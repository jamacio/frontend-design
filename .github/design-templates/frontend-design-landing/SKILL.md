...
description: >
  ...
  collage) — the same aesthetic Frontend Design uses for its own marketing
  surface. The agent fills a typed `inputs.json` from a brand brief,
  optionally generates 16 collage assets via image-gen-2, then runs a
...
parameters:
  ...
  image_strategy:
    ...
    default: placeholder
    description: >
      `generate` calls image-gen-2 (fal.ai or Azure) for all 16 slots.
...
sibling [`frontend-design-landing-deck`](../frontend-design-landing-deck/) skill)
...
This is the canonical Frontend Design marketing-page recipe — the example
output is the very page you see at [frontend-design].
...
worked example (Frontend Design itself).
...
#### `generate` — image-gen-2 mode

```bash
FAL_KEY=... npx tsx scripts/imagegen.ts <inputs.json> --out=<out>/assets/
```

Calls fal.ai's `AI provider/image-gen-2` synchronous endpoint per slot.
...
route them through the `/image-gen-fal` slash-command skill manually.
...
`pnpm --filter @frontend-design/landing-page build` for a static `out/`
...
- [`skills/frontend-design-landing-deck/`](../frontend-design-landing-deck/) — sibling slides skill that reuses this design system.
