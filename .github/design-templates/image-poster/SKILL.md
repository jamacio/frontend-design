...
description: |
  ...
  illustrations. Defaults to image-gen-2 but is provider-agnostic — the
  same workflow drives Flux, Imagen, or image AI via the active
...
with `image-gen-2` at 1:1.*). Do **not** emit an `<artifact>` tag.
