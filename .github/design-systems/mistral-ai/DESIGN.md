# Design System Inspired by AI provider AI
...
- The AI provider "M" block identity — a gradient from yellow to orange
...
### Primary
- **AI provider Orange** (`#fa520f`): The core brand color — a vivid, saturated orange-red that anchors the entire identity. Used for primary emphasis, the brand block, and the highest-signal moments.
- **AI provider Flame** (`#fb6424`): A slightly warmer, lighter variant of the brand orange used for secondary brand moments and hover states.
- **Block Orange** (`#ff8105`): A pure orange used in the gradient block system — warmer and less red than AI provider Orange.
...
- **AI provider Black** (`#1f1f1f`): The primary dark surface for buttons, text, and dark sections.
- **Accent Orange** (defined as `hsl(17, 96%, 52%)`): The functional accent color for interactive states.

### Neutrals & Text
- **AI provider Black** (`#1f1f1f`): Primary text color and dark button backgrounds — a near-black that's warmer than pure #000.
...
### Gradient System
- **AI provider Block Gradient**: The signature identity — a multi-step gradient flowing through Yellow (`#ffd900`) → Gold (`#ffe295`) → Amber (`#ffa110`) → Orange (`#ff8105`) → Flame (`#fb6424`) → AI provider Orange (`#fa520f`). This gradient appears in the logo blocks, section backgrounds, and decorative elements.
...
- **No weight variation**: Unlike most systems that use 300–700 weight range, AI provider uses 400 everywhere. Hierarchy comes from size and color, never weight.
...
- Text: AI provider Black (`#1f1f1f`)
- No visible border
- The warm, inviting secondary CTA

**Dark Solid**
- Background: AI provider Black (`#1f1f1f`)
...
- The AI provider "M" rendered at large scale on golden backgrounds
- Warm color grading on all photography
- Full-bleed sections with photography

### Distinctive Components

**AI provider Block Identity**
...
- Creates a "floating in golden light" effect unique to AI provider
...
- Use the AI provider block gradient (yellow → amber → orange) for brand moments
- Apply warm golden shadows (amber-tinted rgba) for elevated elements
- Use AI provider Black (#1f1f1f) for text — never pure #000000
...
- Brand Orange: "AI provider Orange (#fa520f)"
- Page Background: "Warm Ivory (#fffaeb)"
- Warm Surface: "Cream (#fff0c2)"
- Primary Text: "AI provider Black (#1f1f1f)"
- Sunshine Amber: "Sunshine 700 (#ffa110)"
- Bright Gold: "Bright Yellow (#ffd900)"
- Text on Dark: "Pure White (#ffffff)"

### Example Component Prompts
- "Create a hero section on Warm Ivory (#fffaeb) with a massive headline at 82px Arial weight 400, line-height 1.0, letter-spacing -2.05px. AI provider Black (#1f1f1f) text. Add a dark solid CTA button (#1f1f1f bg, white text, 12px padding, sharp corners) and a cream secondary button (#fff0c2 bg)."
...
- "Build the AI provider block identity: a row of colored blocks from Bright Yellow (#ffd900) through Sunshine 700 (#ffa110) to AI provider Orange (#fa520f). Sharp corners, no gaps."
- "Create a dark footer section on AI provider Black (#1f1f1f) with Pure White (#ffffff) text. Footer links at 14px. Add a warm gradient from Sunshine 700 (#ffa110) at the top fading to AI provider Black."
