...
This system is paired with `image-gen-fal` / `image-gen-azure` via the
frontend-design-landing skill. Several common image-model defaults will
...
See `skills/frontend-design-landing/assets/imagegen-prompts.md` for the
...
- Always include the metadata strip, the side rails, the Roman
  section rules, and a footer with a giant `Frontend Design.` (or brand)
