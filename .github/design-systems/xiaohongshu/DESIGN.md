...
All values below are sampled from production CSS at `#` (inline `<style>` blocks for `:root, .force-light` and `:root[dark], .force-dark`).
...
All values sampled from production CSS at `#`.
